...
/// [hoverPopupProvider]. When the state becomes visible (and the text viewer
/// is in horizontal mode and a novel directory is open), it inserts a
/// [HoverPopupWidget] into the nearest [Overlay] near the pointer position.
/// When the state goes hidden — or the display mode flips to vertical — it
/// removes the entry.
///
/// Vertical mode triggers an automatic hide() on the popup notifier so the
/// state stays consistent: switching back to horizontal does not auto-restore
/// a stale popup.
...
class _HoverPopupHostState extends ConsumerState<HoverPopupHost> {
  ...
  Widget build(BuildContext context) {
    ref.listen<TextDisplayMode>(displayModeProvider, (_, next) {
      if (next != TextDisplayMode.horizontal) {
        // Drop any active popup state so re-entering horizontal mode does
        // not auto-restore it. The hoverPopupProvider listen below will
        // then remove the overlay entry as a downstream effect.
