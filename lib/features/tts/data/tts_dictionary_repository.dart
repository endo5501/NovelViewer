...
class TtsDictionaryRepository {
  ...
  Future<void> deleteEntry(int id) async {
    final db = await _database.database;
    await db.delete('tts_dictionary', where: 'id = ?', whereArgs: [id]);
  }
  ...
  static String applyDictionaryWithEntries(
    List<TtsDictionaryEntry> entries,
    String text,
  ) {
    ...
    while (i < text.length) {
      ...
      for (final entry in entries) {
        ...
        if (i + surface.length <= text.length && text.startsWith(surface, i)) {
