...
typedef OnMarkEnter =
    void Function(String word, Offset position, HoverToken token);
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        Text(
          rubyText,
          style: baseStyle?.copyWith(fontSize: rubyFontSize, height: 1.0),
        ),
        baseWidget,
      ],
    );
  ...
  for (final segment in segments) {
    switch (segment) {
      case PlainTextSegment(:final text):
        ...
        if (hasQuery) {
          renderedSpans = _buildHighlightedPlainSpans(
            text,
            query,
            baseStyle,
...
List<InlineSpan> _applyLocalMarksToSpans(
  ...
  for (final span in renderedSpans) {
    ...
    for (var i = 1; i <= text.length; i++) {
      final markHere = i < text.length ? markAt(positionInSegment + i) : null;
...
List<TextSpan> _applyTtsHighlight(
  ...
  final ttsStyle =
      baseStyle?.copyWith(backgroundColor: _ttsHighlightColor) ??
