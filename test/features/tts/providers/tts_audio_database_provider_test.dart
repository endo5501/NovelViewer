...
void main() {
  ...
  group('ttsAudioDatabaseProvider family', () {
    test('returns the same instance for the same folder path', () {
      ...
      final db1 = container.read(ttsAudioDatabaseProvider(tempDir.path));
    ...
    test('returns different instances for different folder paths', () {
      ...
      final dbA = container.read(ttsAudioDatabaseProvider(tempDir.path));
      final dbB = container.read(ttsAudioDatabaseProvider(tempDir2.path));
      expect(identical(dbA, dbB), isFalse);
    });

    test(
      'registry.closeAll releases the handle; a re-read yields a new one',
      () async {
        final container = ProviderContainer();
        addDbContainerTearDown(container);
    ...
    test(
      'awaited container disposal releases the file lock on the folder',
      () async {
        final container = ProviderContainer();
