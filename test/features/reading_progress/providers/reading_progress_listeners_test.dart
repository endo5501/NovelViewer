...
class _UpsertCall {
  ...
  const _UpsertCall({required this.novelId, required this.fileName});
}
...
class _FakeRepository implements ReadingProgressRepository {
  ...
  _FakeRepository({this.onFindByNovelId = _alwaysNull});
  ...
  Future<void> upsert({
    ...
    upsertCalls.add(_UpsertCall(novelId: novelId, fileName: fileName));
  }
...
void main() {
  group('readingProgressAutoSaveListenerProvider', () {
    test(
      'upserts when selectedFile transitions to a non-null entry inside a novel folder',
      () async {
    ...
    test(
      'bumps the reading-progress revision after a successful save',
      () async {
    ...
    test('upserts under the registered leaf name for a nested novel folder '
    ...
    test('does not upsert inside an unregistered organizational folder '
    ...
    test(
      'does not upsert at the library root (no novel id can be resolved)',
      () async {
        final repo = _FakeRepository();
        final container = _buildContainer(
          repository: repo,
    ...
    test(
      'repository failure is logged but does not crash the listener',
      () async {
        final repo = _FakeRepository();
  ...
  group('readingProgressAutoOpenListenerProvider', () {
    ...
    test(
      'entering a novel folder with stored progress selects that file',
      () async {
        final repo = _FakeRepository(
    ...
    test(
      ...
      () async {
        ...
        expect(
          ...
          movedChapter3.path,
          reason:
              'auto-open SHALL match by file_name in the current folder, '
              'ignoring the stale stored location',
        );
      },
    );

    test('entering a nested novel folder resolves the registered leaf id '
        '(F106 regression)', () async {
    ...
    test(
      'does nothing when the stored file is no longer in the directory',
      () async {
        final repo = _FakeRepository(
    ...
    test(
      'does not override an existing selection belonging to this novel',
      () async {
        final repo = _FakeRepository(
          onFindByNovelId: (id) => ReadingProgress(
            novelId: 'narou_n1234ab',
    ...
    test(
      'repository read failure is logged and selection is left untouched',
      () async {
        final repo = _FakeRepository();
    ...
    test('re-entering the same folder after the user changed selection picks up '
      ...
      await Future<void>.delayed(Duration.zero);
      expect(
        repo.upsertCalls.map((c) => c.fileName),
        contains(chapter5.name),
