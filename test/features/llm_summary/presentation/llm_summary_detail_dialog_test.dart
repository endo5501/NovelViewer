...
void main() {
  ...
  group('facts tab', () {
    testWidgets('lists files in ascending order with their facts', (
      ...
      await tester.pumpWidget(
        _harness(
          facts: [_fact('002.txt', '・王国出身'), _fact('001.txt', '・髪は金色')],
          snapshots: [_snap(10, '要約#10')],
        ),
      );
      await tester.pumpAndSettle();
    ...
    testWidgets('keeps invalidated facts in the list, greyed with a badge', (
      ...
      await tester.pumpWidget(
        _harness(
          facts: [
            _fact('001.txt', '・有効な事実'),
            _fact('002.txt', '・無効な事実', hash: FactCacheRepository.sentinelHash),
          ],
          snapshots: [_snap(10, '要約#10')],
        ),
      );
      ...
      final opacity = tester.widget<Opacity>(
        find.ancestor(of: find.text('・無効な事実'), matching: find.byType(Opacity)),
      );
      expect(opacity.opacity, lessThan(1.0));

      // Valid section is NOT greyed (no low-opacity Opacity ancestor).
      expect(
        find.ancestor(of: find.text('・有効な事実'), matching: find.byType(Opacity)),
        findsNothing,
      );
    });
  });
  ...
  group('result tab', () {
    testWidgets('shows the latest snapshot and navigates between snapshots', (
      ...
      await tester.tap(find.byKey(const Key('history_detail_snapshot_prev')));
      await tester.pumpAndSettle();
      expect(find.text('要約#10'), findsOneWidget);
    });

    testWidgets('shows empty message when no snapshots exist', (tester) async {
      await tester.pumpWidget(_harness(facts: const [], snapshots: const []));
      await tester.pumpAndSettle();
