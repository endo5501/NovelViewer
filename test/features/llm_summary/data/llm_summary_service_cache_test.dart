...
void main() {
  ...
  group('LlmSummaryService fact-cache orchestration', () {
    test(
      'reuses cached facts for prior files and extracts only the misses',
      () async {
        for (var i = 1; i <= 7; i++) {
    ...
    test(
      'changed file content (hash mismatch) re-extracts only that file',
      () async {
        await createFile('001_ch.txt', 'アリスが登場した。');
        await createFile('002_ch.txt', 'アリスが旅に出た。');
    ...
    test('re-analysis at an existing covered invalidates the word cache and '
        're-extracts', () async {
      await createFile('001_ch.txt', 'アリスが登場した。');
      await createFile('002_ch.txt', 'アリスが旅に出た。');
    ...
    test(
      'analysis at a NEW covered reuses cache (not treated as re-analysis)',
      () async {
        await createFile('001_ch.txt', 'アリスが登場した。');
        await createFile('002_ch.txt', 'アリスが旅に出た。');
    ...
    test(
      'round-1 progress counters reflect only the extracted (miss) files',
      () async {
        for (var i = 1; i <= 3; i++) {
    ...
    test(
      'extracted files are written back to the cache with current hash',
      () async {
        await createFile('001_ch.txt', 'アリスが登場した。');
