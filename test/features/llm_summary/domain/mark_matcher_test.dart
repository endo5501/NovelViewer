...
void main() {
  group('findMarks', () {
    ...
    test(
      'excludes 1-character cached words via the default min-length filter',
      () {
        final marks = findMarks(
          text: 'のもアの',
    ...
    test(
      'longest match wins when two cached words overlap at the same start',
      () {
        final marks = findMarks(
          text: 'アリスの剣を持って',
          wordsByStyle: const {
    ...
    test('non-overlapping different words are all marked independently', () {
      final marks = findMarks(
        text: 'アリスは聖印を持って',
        wordsByStyle: const {'アリス': MarkStyle.solid, '聖印': MarkStyle.solid},
      );
      expect(marks, hasLength(2));
    ...
    test(
      'substring of an unrelated word is still marked (accepted limitation)',
      () {
        final marks = findMarks(
          text: 'メアリスが歩く',
          wordsByStyle: const {'アリス': MarkStyle.solid},
    ...
    test('returns empty when there are no cached words to mark', () {
      final marks = findMarks(text: 'アリスは王女', wordsByStyle: const {});
      expect(marks, isEmpty);
    });
    ...
    test('skips marks placed inside ruby tag annotations (rt content)', () {
      final marks = findMarksOnBaseText(
        ...
        wordsByStyle: const {'聖印': MarkStyle.solid, 'せいいん': MarkStyle.solid},
      );
