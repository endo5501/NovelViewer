...
void main() {
  group(
    'VerticalTextViewer hover callbacks — passthrough to VerticalTextPage',
    () {
      testWidgets(
        'forwards onMarkEnter / onMarkExit / onHoverHideRequest props',
        (tester) async {
      ...
      testWidgets('passes null callbacks through unchanged', (tester) async {
        await tester.pumpWidget(
          _build(segments: const [PlainTextSegment('アリスが歩く')]),
        );
        await tester.pumpAndSettle();
  ...
  group(
    'VerticalTextViewer._changePage — fires hide alongside selection clear',
    () {
      testWidgets(
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        void Function()? onHide() =>
            () => hideCount++;
        Widget buildWith({int? target}) => ProviderScope(
          child: MaterialApp(
            locale: const Locale('ja'),
            localizationsDelegates: AppLocalizations.localizationsDelegates,
            supportedLocales: AppLocalizations.supportedLocales,
