    ...
    child: MaterialApp(
      ...
      home: Center(
        child: ConstrainedBox(
          ...
          child: VerticalTextViewer(segments: segments, baseStyle: baseStyle),
        ),
      ),
    ),
  );
  ...
  void visitChildren(RenderObject parent) {
    parent.visitChildren((child) {
      if (child is RenderBox && child.hasSize) {
        ...
        expect(
          childLeft,
          greaterThanOrEqualTo(wrapLeft - 1.0),
          reason:
        ...
        expect(
          childRight,
          lessThanOrEqualTo(wrapRight + 1.0),
          reason:
...
void main() {
  group('Vertical text pagination with font size changes', () {
    testWidgets('large font size produces more pages than small font size', (
      tester,
    ) async {
      ...
      expect(
        largeFontPages!,
        greaterThan(smallFontPages!),
        reason: 'Larger font should produce more pages',
      );
    });

    testWidgets('text does not overflow container with large font size', (
      tester,
    ) async {
  ...
  group('Vertical text pagination empty columns', () {
    testWidgets('text with blank lines uses available width efficiently', (
      tester,
    ) async {
      ...
      expect(
        wrapBox.size.width,
        greaterThan(availableWidth * 0.9),
        reason:
    ...
    testWidgets('text with blank lines still does not overflow', (
      tester,
    ) async {
      final lines = <String>[];
      for (var i = 0; i < 100; i++) {
  ...
  group('Vertical text pagination empty columns take full width', () {
    testWidgets('blank lines increase page count proportionally', (
      tester,
    ) async {
      ...
      expect(
        pagesWithBlanks,
        greaterThanOrEqualTo((pagesNoBlanks * 1.5).floor()),
        reason:
    ...
    testWidgets('empty columns are not packed more densely than text columns', (
      tester,
    ) async {
  ...
  group('Vertical text pagination empty columns edge cases', () {
    testWidgets('all empty columns does not crash and stays bounded', (
      tester,
    ) async {
    ...
    testWidgets('single empty line remains single page', (tester) async {
      await tester.pumpWidget(
        _buildTestWidget(
  ...
  group('Vertical text pagination empty columns regression', () {
    testWidgets('text without blank lines still does not overflow', (
      tester,
    ) async {
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        expect(
          pagesWithBlanks,
          lessThanOrEqualTo(pagesNoBlanks * 2),
          reason:
    ...
    testWidgets('targetLineNumber navigates correctly with blank lines', (
      tester,
    ) async {
  ...
  group('Vertical text pagination with font family changes', () {
    testWidgets('pagination recalculates when font family changes', (
      tester,
    ) async {
      final longText = 'あ' * 200;
      final segments = [PlainTextSegment(longText)];
