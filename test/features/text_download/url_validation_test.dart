...
void main() {
  ...
  group('URL validation', () {
    test('accepts ncode.syosetu.com URLs', () {
      final site = registry.findSite(
        Uri.parse('https://ncode.syosetu.com/n9669bk/'),
      );
    ...
    test('accepts novel18.syosetu.com URLs', () {
      final site = registry.findSite(
        Uri.parse('https://novel18.syosetu.com/n1234ab/'),
      );
      expect(site, isNotNull);
    });
    ...
    test('resolves unknown domains to the generic web fallback', () {
      ...
      final site = registry.findSite(
        Uri.parse('https://example.com/novel/123'),
      );
      expect(site, isNotNull);
    ...
    test('accepts kakuyomu URL with episode path', () {
      final site = registry.findSite(
        Uri.parse('https://kakuyomu.jp/works/1177354054881162325/episodes/1'),
      );
      expect(site, isNotNull);
    });
  });
  ...
  group('URL validation hardening (F119)', () {
    ...
    test(
      'look-alike Kakuyomu host is not treated as Kakuyomu (web fallback)',
      () {
        final site = registry.findSite(
          Uri.parse('https://kakuyomu.jp.evil.com/works/123'),
        );
        expect(site, isNotNull);
        expect(site!.siteType, 'web');
      },
    ...
    test('accepts www.kakuyomu.jp host', () {
      final site = registry.findSite(
        Uri.parse('https://www.kakuyomu.jp/works/123'),
      );
      expect(site, isNotNull);
    ...
    test(
      'Kakuyomu URL where /works/ is not the leading segment falls back to web',
      () {
        final site = registry.findSite(
          Uri.parse('https://kakuyomu.jp/foo/works/123'),
        );
        expect(site, isNotNull);
        expect(site!.siteType, 'web');
      },
    ...
    test('Kakuyomu URL with a non-numeric work id falls back to web', () {
      final site = registry.findSite(
        Uri.parse('https://kakuyomu.jp/works/123abc'),
      );
      expect(site, isNotNull);
      expect(site!.siteType, 'web');
    });
    ...
    test('rejects a non-web scheme (ftp)', () {
      final site = registry.findSite(Uri.parse('ftp://kakuyomu.jp/works/123'));
      expect(site, isNull);
    });

    test(
      'accepts http URL for a supported host (upgraded to https on normalize)',
      () {
        final url = Uri.parse(
          'http://www.aozora.gr.jp/cards/001779/files/x.html',
        );
