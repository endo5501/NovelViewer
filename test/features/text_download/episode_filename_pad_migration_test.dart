...
void main() {
  ...
  group('migrateEpisodeFileNamePadding', () {
    test(
      'pad width increase (99 -> 100) renames 2-digit files to 3-digit',
      () async {
        // Existing library: 99 episodes at pad width 2.
        for (var i = 1; i <= 99; i++) {
          File(
            '${dir.path}/${formatEpisodeFileName(i, '第$i話', 99)}',
    ...
    test(
      'pad width decrease (100 -> 99) renames 3-digit files to 2-digit',
      () async {
        for (var i = 1; i <= 99; i++) {
          File(
            '${dir.path}/${formatEpisodeFileName(i, '第$i話', 100)}',
          ).writeAsStringSync('content $i');
        }
    ...
    test(
      'residual old-width duplicate is deleted, canonical file untouched',
      () async {
    ...
    test(
      'is a no-op when filenames already match the current pad width',
      () async {
        for (var i = 1; i <= 100; i++) {
          File(
            '${dir.path}/${formatEpisodeFileName(i, '第$i話', 100)}',
    ...
    test(
      'does not migrate a title-changed file when no cache entry exists',
      () async {
    ...
    test(
      'migrates a title-changed file when the cache records the old title',
      () async {
        ...
        expect(
          File('${dir.path}/001_新しいタイトル.txt').readAsStringSync(),
          'old content',
        );
      },
    );
    ...
    test(
      'deletes the stale title-changed file when the new name already exists',
      () async {
    ...
    test(
      'does not claim a same-titled file written for a different episode',
      () async {
        ...
        expect(
          File('${dir.path}/05_閑話.txt').readAsStringSync(),
          'episode X content',
        );
      },
    );
    ...
    test(
      'does not pad-migrate a same-titled file of a shifted episode',
      () async {
        ...
        File(
          '${dir.path}/05_閑話.txt',
        ).writeAsStringSync('other episode content');
    ...
    test(
      'leaves a title-changed file alone when the cached title differs too',
      () async {
  ...
  group('Download skip detection after pad migration', () {
    ...
    test(
      '99->100 boundary: episodes 1-99 are skipped, only 100 is downloaded',
      () async {
        ...
        for (var i = 1; i <= 99; i++) {
          ...
          File(
            '${novelDir.path}/${formatEpisodeFileName(i, title, 99)}',
