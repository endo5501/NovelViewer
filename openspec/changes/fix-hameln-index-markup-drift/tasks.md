...
- [x] 5.2 既存のダウンロード済みハーメルン作品を更新し、全話再取得が走ること・旧ファイル名が残らないこと（同じ話が2件に見えないこと）を確認する（**未実施**: 旧マークアップ時代にダウンロード済みのライブラリが本開発環境に存在しないため実機確認不能。ロジックは `migrateEpisodeFileNamePadding` のユニットテスト（タイトル変更時のリネーム／旧ファイル削除／誤掴み防止）でカバー済み）
- [x] 5.3 短編作品をダウンロードし、本文とタイトルが正しく取得できることを確認する（実サイト生HTML `/novel/123475/` で検証: タイトル「異世界転生？他所でやれ！！」・本文5075文字・後書き除外を確認）
- [x] 5.4 R-18作品が引き続き取得できることを確認する（`over18` Cookie 経路の回帰確認）（R-18タグ作品3件を `over18=off` 付きで取得し、目次が81/61/43話として解析できることを確認。Cookie 有無で差が出る「年齢確認インタースティシャルで実際にゲートされた作品」の検体は見つけられなかったため、ゲート解除そのものの再現確認は未実施。requestHeaders は本changeで無変更）
...
- [x] 7.1 code-reviewスキルを使用してコードレビューを実施（指摘2件をTDDで修正: タイトルspan欠落時のフォールバック汚染 / パッド幅経路の同名タイトル誤掴み）
- [x] 7.2 codexスキルを使用して現在開発中のコードレビューを実施（同じ2件を独立に指摘。目次ページネーションの仮説指摘は、333話の作品も1ページ配信であること・旧実装も未対応であることから見送り）
