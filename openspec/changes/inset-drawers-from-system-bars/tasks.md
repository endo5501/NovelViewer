...
- [x] 1.8 失敗するテストをコミットする

## 2. 実装

- [x] 2.1 `lib/home_screen.dart` の `drawer` で `LeftColumnPanel` を `SafeArea` で包む
- [x] 2.2 `lib/home_screen.dart` の `endDrawer` で `SearchResultsPanel` を `SafeArea` で包む
- [x] 2.3 なぜドロワーだけに必要でパネル側に置かないのかを、既存のコメントの書き方に合わせて残す
- [x] 2.4 `fvm flutter test test/home_screen_adaptive_shell_test.dart` を実行し、新しいテストが通ることを確認する
- [x] 2.5 既存のドロワー関連テストが引き続き通ることを確認する
