...
### Requirement: ユーザデータディレクトリの保護
インストーラおよびアンインストーラは `{app}\NovelViewer\` サブフォルダ（ユーザ作成データ）を作成・変更・削除してはならない（MUST NOT）。

#### Scenario: 上書きインストールでデータが残る
- **GIVEN** ユーザが小説を複数ダウンロード済みで、`%LOCALAPPDATA%\Programs\NovelViewer\NovelViewer\` に sqlite DB とテキストファイルが存在する
- **WHEN** 新バージョンのインストーラを実行する
- **THEN** `{app}\NovelViewer\` 配下の全ファイルが変更されずに残る

#### Scenario: アンインストールでデータが残る
- **WHEN** ユーザがアンインストールを実行する
- **THEN** `{app}\NovelViewer\` サブフォルダとその中身は削除されない
