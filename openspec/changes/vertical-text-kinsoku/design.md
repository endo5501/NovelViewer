...
### 4. 禁則の連鎖への対応：最大1文字の調整に制限

**選択**: 禁則処理による調整は最大1文字とする。連鎖的な禁則文字（例: `。」`）が続く場合でも、1文字のみ前のカラムに含める。

**理由**: 実用上、小説テキストで2文字以上の禁則文字が連続するケースは稀（`。」` は閉じ括弧の後に句点は来ない構造）。1文字制限によりカラム幅のばらつきを最小限に抑え、レイアウトの安定性を保つ。万が一の連鎖ケースでは2文字目以降は次のカラム先頭に配置されるが、実用上の影響は軽微。
