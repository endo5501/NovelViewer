...
診断項目からエンドポイントURLとモデルパスを外しても、`cause` と `stackTrace` は生のテキストをそのまま通す。実際に次の経路で漏れる。

- `ClientException.toString()` は `uri=` を含むため、自宅のOllamaのIPとポートが入る
- OpenAI互換クライアントはエラー応答ボディを丸ごと例外に埋め込む
- Irodoriのネイティブ層は `could not open audio input: <絶対パス>` を返す

いずれも今回の変更以前から表示されていたが、コピー経路を足したことでその文字列は報告として外に出る前提になる。エンドポイントを伏せるという決定が cause 側で崩れるため、描画の直前に伏せ字をかける。

規則は3つ。URLは `scheme://` から空白までを丸ごと `<endpoint>` に置換する。認証情報がuserinfoに入っていても同時に消える。絶対パスはディレクトリ部分を `<path>` に畳み、ファイル名だけ残す。どちらのファイルかは追えて、どこに置いてあるかは出ない。

3つ目はURL以外に現れるホストである。`SocketException` はアドレスをURIとは別に `address = 192.168.99.99` として持つため、URLの規則では捕まらない。iOSシミュレータでの確認で実際に残っているのを見つけた。`address =` に続く値と、素のIPv4リテラルを `<address>` に置換する。バージョン文字列を誤爆しないよう、IPv4は4つ組にのみ一致させる。

かける場所は `renderFailureReport` と `formatFailureSnackBarBody` の2箇所、すなわちユーザーの目に触れるすべての出力とする。個々の生成元に任せると、後から足された失敗経路が忘れる。

*代替案*: 設定済みの `baseUrl` の文字列一致だけを消す。却下。リダイレクト先や別ホストへの接続失敗を取りこぼす。
...
**伏せ字が過剰に効く** → 絶対パスに見える文字列がエラー本文の一部だった場合、ディレクトリ部分が畳まれる。原因の特定にパスの位置そのものが要る場面は考えにくく、ファイル名は残るため、取り違えの危険より漏洩の危険を優先する。
