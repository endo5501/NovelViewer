...
- [x] 9.1 `SocketException` が `address = <host>` として別に持つアドレスが伏せ字を抜けることを検証するテストを追加する
- [x] 9.2 素のIPv4リテラルと、バージョン文字列を誤爆しないことのテストを追加する
- [x] 9.3 テストを実行して失敗を確認し、コミットする
- [x] 9.4 `redactSensitive` にアドレスの規則を足す
- [x] 9.5 テストが通ることを確認する

## 10. verifyでの指摘

- [x] 10.1 design.md の診断項目表を実装に合わせる（`scope` → `covered up to`、`segment` の追加、モデルの出所）
- [x] 10.2 `TtsEditDialog` の失敗提示を `showSynthesisFailure` として切り出し、テストから呼べるようにする
- [x] 10.3 モーダルの内側にメッセンジャーがあること、失敗通知の詳細アクションが押せること、診断情報が揃うことを検証するテストを追加する
- [x] 10.4 メッセンジャーの受け渡しを外すとテストが落ちることを確認する
...
- [x] 6.6 iOS実機またはシミュレータで、LLM解析を意図的に失敗させ、スナックバーが残り詳細ダイアログからコピーできることを確認
