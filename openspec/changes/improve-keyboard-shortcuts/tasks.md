...
- [ ] 3.1 ファイルブラウザ（`LeftColumnPanel`）と小説画面（`TextViewerPanel`）に名前付き`FocusNode`を与え、`switchPane`で2ペイン間をトグルするテストを書く（TDD）
- [ ] 3.2 `switchPane`（既定Tab）を実装し、AppBarボタン群をTab巡回から除外する（`FocusTraversalGroup`/`skipTraversal`相当）
- [ ] 3.3 検索入力フィールドにフォーカスがある間はペイン切替が抑制されることのテストを書き、実装する
...
- [ ] 5.1 アプリ起動直後にファイルブラウザへフォーカスが当たることのテストを書く（autofocus対象をファイルブラウザにする、TDD）
- [ ] 5.2 `switchPane`（Tab）でファイルブラウザへ確実にフォーカスが入ることのテストを書き、実装する（`LeftColumnPanel`がペイン用`FocusNode`を受け入れる）
- [ ] 5.3 ファイルブラウザにフォーカスがある間、矢印キーがページ送りに横取りされず標準のフォーカス操作が機能することを確認する（独自ナビは新設しない）

## 6. Ctrl+Fの改善（確実フォーカス・トグル閉じ）

- [ ] 6.1 右カラム非表示からのCtrl+Fで検索入力欄へ初回フォーカスが当たるテストを書く（mount時の現在値チェック/autofocus、`listenManual`初回未発火バグの回帰防止、TDD）
- [ ] 6.2 確実なフォーカスを実装する
- [ ] 6.3 2度目のCtrl+Fで検索ボックス非表示＋クエリ/結果クリア＋右カラムを閉じるトグル動作のテストを書き、実装する（2.3の共通クローズ関数を使用）
- [ ] 6.4 検索欄フォーカス時のEsc（`_onEscape`）が同じ共通クローズ関数を呼び、右カラムごと閉じてフォーカスをメインに戻すテストを書き、実装する
- [ ] 6.5 テキスト選択時の即時検索（従来動作）が維持されることを確認する
