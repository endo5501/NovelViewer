...
- [x] 8.1 設定ダイアログに「ショートカット」セクションを追加し、各論理アクションの現在割り当てを一覧表示するテストを書く（TDD）
- [x] 8.2 キー入力をキャプチャして再割当するUIと、重複割り当ての拒否（再割当不可・既存維持）＋通知を実装する
- [x] 8.3 「既定に戻す」操作をUIから実行できるようにする
- [x] 8.4 i18n文言（ja/en/zh）を追加する
