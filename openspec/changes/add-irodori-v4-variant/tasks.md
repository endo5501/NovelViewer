...
- [ ] 1.1 Hugging Face `endo5501/audio.cpp` へ `Irodori-TTS-v4-Small-GGUF/irodori-tts-v4-small-f16.gguf` (1,762,161,536 バイト) をミラーする
- [ ] 1.2 Hugging Face `endo5501/audio.cpp` へ `Irodori-TTS-600M-v3-VoiceDesign-GGUF/irodori-tts-600m-v3-voicedesign-f16.gguf` (1,463,787,680 バイト) をミラーする
- [ ] 1.3 ミラー後の実サイズが上記 pin 値と一致することを確認する
...
- [ ] 3.8 macOS で `scripts/build_irodori_macos.sh` を実行しビルドできることを確認する
...
- [ ] 6.1 variant ごとの単一 GGUF パスと pin サイズを検証する失敗するテストを書く
- [ ] 6.2 ダウンロード済み判定が variant ごとに独立していることを検証する失敗するテストを書く
- [ ] 6.3 サイズが pin 値と一致しない場合にファイルが削除されエラーになることを検証するテストを書く (既存動作の維持)
- [ ] 6.4 キャンセルと再試行が従来どおり動作することを検証するテストを書く (既存動作の維持)
- [ ] 6.5 6.1〜6.4 のテストが失敗することを確認してコミットする
- [ ] 6.6 `IrodoriModelDownloadService` を variant ごとの単一 GGUF ダウンロードへ変更する
- [ ] 6.7 `irodori_model_download_providers` のダウンロード状態を variant 単位へ変更する
- [ ] 6.8 `tts_model_readiness_provider` を variant を考慮した判定へ変更する
- [ ] 6.9 `IrodoriTtsEngine.loadModel()` へ渡すモデルディレクトリを variant ごとの GGUF ディレクトリへ変更する
- [ ] 6.10 6.1〜6.4 のテストが通ることを確認する

## 7. 旧 safetensors 資産の後始末 (TDD)

- [ ] 7.1 旧形式3ディレクトリの検出と、解放可能容量の算出を検証する失敗するテストを書く
- [ ] 7.2 旧資産の存在が新形式のダウンロード済み判定に影響しないことを検証するテストを書く
- [ ] 7.3 ユーザ操作による削除で3ディレクトリが削除され、新形式の GGUF が影響を受けないことを検証する失敗するテストを書く
- [ ] 7.4 確認なしには削除されないことを検証するテストを書く
- [ ] 7.5 7.1〜7.4 のテストが失敗することを確認してコミットする
- [ ] 7.6 旧資産の検出・容量算出・削除を実装する
- [ ] 7.7 7.1〜7.4 のテストが通ることを確認する

## 8. UI と l10n (TDD)

- [ ] 8.1 variant 選択 UI が表示され、選択がプロバイダへ反映されることを検証する失敗するテストを書く
- [ ] 8.2 v4 選択時に caption guidance scale の UI が無効化され、理由が表示されることを検証する失敗するテストを書く
- [ ] 8.3 v3 に戻すと caption 系 UI が復帰することを検証するテストを書く
- [ ] 8.4 旧資産が残っているときに削除導線が表示されることを検証する失敗するテストを書く
- [ ] 8.5 追加した文言が ja / en / zh の全ロケールに存在することを検証するテストを書く
- [ ] 8.6 8.1〜8.5 のテストが失敗することを確認してコミットする
- [ ] 8.7 `irodori_settings_section.dart` に variant 選択 UI を実装する
- [ ] 8.8 v4 選択時の caption 系 UI 無効化と理由表示を実装する
- [ ] 8.9 旧資産の削除導線を実装する
- [ ] 8.10 ja / en / zh の文言を `lib/l10n/` へ追加する
- [ ] 8.11 8.1〜8.5 のテストが通ることを確認する

## 9. 実機確認

- [ ] 9.1 v3 を選択してモデルをダウンロードし、caption ありの合成が従来どおり動作することを確認する
- [ ] 9.2 v4 を選択してモデルをダウンロードし、参照音声のみの合成がクリーンであることを確認する
- [ ] 9.3 v4 選択中に memo を記入したセグメントを合成し、末尾に余計な発話が付かないことを確認する
- [ ] 9.4 variant を切り替えた際にモデルが再ロードされることを確認する
- [ ] 9.5 旧 safetensors 資産の検出・削除が動作することを確認する
