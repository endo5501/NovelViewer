...
- [ ] 2.1 `vertical_text_viewer` のテストに「同一 `(segments, constraints, style, columnSpacing)` での再ビルドで再ページネーションが走らない」回数スパイテストを追加。実行して失敗を確認
- [ ] 2.2 「`constraints` 変化／`style` 変化でページが再計算される」「`bookmarkLineNumbers` のみ変化・`targetLine` のみ変化では重い層が再計算されないが、bookmarkPages/targetPage は更新される」同値性テストを追加。実行して失敗を確認
- [ ] 2.3 `_paginateLines` を「重い層」（pages/pageStarts/lineBreakIndicesPerPage/charOffsetPerPage/lineStartColumns）と「軽い層」（targetPage/bookmarkPages/firstLinePerPage）に分割するヘルパへリファクタ（挙動を変えずに分離のみ）。既存テスト緑を確認
- [ ] 2.4 重い層のキャッシュフィールド（`_cachedHeavy` と key: `_cachedSegments`/`_cachedConstraints`/`_cachedPaginateStyle`/`_cachedColumnSpacing`）を State に追加。`segments` は identity、他は値比較でキー一致時に再利用
- [ ] 2.5 軽い層は毎ビルド、キャッシュ済み重い層から再計算して `_PaginationResult` を組む（キャッシュヒット時も `_PaginationResult` は毎ビルド生成し、build 内フィールド代入経路 `_pageCount`/`_currentPageSegments` を不変に保つ）。2.1/2.2 のテストを通す
- [ ] 2.6 `didUpdateWidget`/`initState` での `_lines`・`_charEntries`・columns 再構築タイミングを確認し、それらに連動して重い層キャッシュが無効化されることを保証（`_cachedPainter`/`_cachedStyle` の既存無効化との整合も確認）
- [ ] 2.7 キー/ホイール/スワイプ/初期ページ/フォント変更/アニメーション系テスト（`vertical_text_viewer_*_test.dart` 群）を実行して回帰がないことを確認
- [ ] 2.8 F115 をコミット
