...
- 「短すぎる」と判定する最小本文文字数（CMSサンプルで決定。例: 200字）。
- コレクション名スラッグの生成規則（日本語をどう safe 化するか、衝突サフィックスの形式）。
- 固定パディング幅の桁数（4桁=9999話で十分か）。
- URL正規化でトラッキングパラメータ（`utm_*`, `fbclid` 等）を除去するか（記事同一性に影響。除去するとクエリで内容が変わるページを誤って同一視するリスク）。
- 密度スコアのリンク減点係数 `k` と候補要素の絞り込み初期値。
- タイトルのサイト名サフィックス除去をどこまで積極的にやるか（`og:title` 優先で多くは回避できる想定）。
