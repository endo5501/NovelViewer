...
- [ ] 1.1 `test/shared/database/db_connection_gate_test.dart` を新規作成し、以下を**失敗するテスト**として書く（実装前）
- [ ] 1.2 並行する複数 getter が単一の `opener` 実行を共有し同一ハンドルを受け取る（二重 open 防止）テスト
- [ ] 1.3 close が in-flight open の決着を待ってからその同一ハンドルを閉じ、close 後にハンドルが残らないテスト
- [ ] 1.4 close 実行中の getter が `DatabaseClosingException` を投げ、新規 open を開始しないテスト（B案）
- [ ] 1.5 open 失敗 Future を非キャッシュ（次回 getter で再試行）／close 完了後に再 open されるテスト
- [ ] 1.6 `fvm flutter test` で 1.2–1.5 がすべて失敗することを確認（Red）

## 2. 接続ゲートの実装（F124・TDD Green）

- [ ] 2.1 `lib/shared/database/database_closing_exception.dart`（または既存 shared 例外群）に `DatabaseClosingException` を新設
- [ ] 2.2 `lib/shared/database/db_connection_gate.dart` に `DbConnectionGate`（`opener` 受け取り・`_open` 単一 Future キャッシュ・`_closing` フラグ）を実装
- [ ] 2.3 `_openOnce` で open 失敗時に `_open=null` にして rethrow（非キャッシュ）、`opener` への再入禁止を doc コメントで明記
- [ ] 2.4 `fvm flutter test` で 1.2–1.5 が緑になることを確認（Green）

## 3. 各ラッパーのゲート移行（F124）

- [ ] 3.1 `TtsAudioDatabase` を `Database? _database` から `DbConnectionGate` 委譲へ移行（`database` getter / `close()` をゲートへ）
- [ ] 3.2 既存の `TtsAudioDatabase` テストが緑のままであることを確認
- [ ] 3.3 `EpisodeCacheDatabase` をゲート委譲へ移行
- [ ] 3.4 `TtsDictionaryDatabase` をゲート委譲へ移行
- [ ] 3.5 `NovelDatabase` をゲート委譲へ移行（**F166 の NUL バイト dedup キー領域には触れない**）
- [ ] 3.6 `fvm flutter test` で全 DB ラッパー関連テストが緑であることを確認
