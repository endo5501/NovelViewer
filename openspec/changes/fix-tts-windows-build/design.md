...
qwen3-tts.cppのCMakeLists.txtはGNU/Clangコンパイラのみを想定して書かれており、MSVC固有の動作差異に対応していない。具体的には以下の3点が問題：
...
- **ビルド後のリンクエラーの可能性** → MSVCのマルチコンフィグビルドでは、GGMLのライブラリが`Release/`サブディレクトリに生成される。既存の`target_link_directories`が正しく解決するか、ビルド実行時に確認が必要。問題が発生した場合は追加対応する。
