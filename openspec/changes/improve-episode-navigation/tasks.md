...
- [ ] 7.4 統合シナリオ手動確認: 200 話のサンプル小説で「ファイル選択 → AppBar とハイライトが追従」「最終ページ 2 回送り → 次話」「先頭ページ 2 回戻し → 前話の末尾」「横書きボタン → 次/前話遷移」「最終話・最初話の端では何も起きない」を順に確認する（実機確認 — ユーザー実行）
- [ ] 7.5 ダークモード手動確認: テーマをダークに切替えて 3.1〜3.3 と 4.6 の視認性を実機で確認する（実機確認 — ユーザー実行）
