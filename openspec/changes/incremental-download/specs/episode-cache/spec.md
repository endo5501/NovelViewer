...
The episode cache database SHALL store episode metadata with the following schema: `url` (TEXT, primary key), `episode_index` (INTEGER), `title` (TEXT), `last_modified` (TEXT, nullable), `downloaded_at` (TEXT).

#### Scenario: Cache entry is stored after download
- **WHEN** an episode is successfully downloaded
- **THEN** the system stores a record with the episode's URL, index, title, the HTTP `Last-Modified` header value (if present), and the current timestamp as `downloaded_at`
...
- **THEN** the existing cache record is replaced with updated `last_modified` and `downloaded_at` values

#### Scenario: Last-Modified is not provided by server
- **WHEN** the server does not include a `Last-Modified` header in the response
