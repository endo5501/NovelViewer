...
- [ ] 1.1 `TtsEditController.generateSegment` に対する失敗するテストを書く
...
- [ ] 1.2 テストが失敗することを確認し、テストのみをコミットする
- [ ] 1.3 `generateSegment` に `resolveRefWavPath` オプション引数を追加し、`TtsRefWavResolver.resolve` へ渡す
- [ ] 1.4 `tts_edit_dialog.dart` の呼び出しで `voiceService?.resolveVoiceFilePath` を渡す
- [ ] 1.5 同ファイルの Qwen3 限定の `copyWithRefWavPath` 暫定処理を削除する
- [ ] 1.6 テストが通ることを確認する
