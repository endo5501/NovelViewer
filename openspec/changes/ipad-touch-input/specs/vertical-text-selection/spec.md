...
指によるタップという入口は、マウスの副ボタンを持たない端末のために設けるものであり、ポインタ種別が touch の場合に限らなければならない（SHALL）。それ以外のポインタによるタップは従来どおり選択解除として扱わなければならない（SHALL）。
