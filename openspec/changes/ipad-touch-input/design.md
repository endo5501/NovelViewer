...
探索段階で 5 点が決着し（スコープ = ①②③、縦書きのトリガ = 選択範囲内タップ、リストの長押し = 分岐なし、縦書きのタップ = タッチのみ、ホバーポップアップ = 対象外）、うち 2 点はレビュー後に前提が誤っていたことが判明して差し替えた。

- 「リストの長押し = 分岐なし」→ **副ボタンを持たないポインタに限定**（D2b）。前提としていた「マウスは何も失わない」が誤りだった。
- 「解析履歴パネルは対象外」→ **長押しを追加**。iPad では非表示だがタッチスクリーン付きデスクトップでは到達手段が無く、新設仕様の文言と矛盾していた。

いずれもユーザーに前提の誤りを報告したうえで再確認を取っている。
