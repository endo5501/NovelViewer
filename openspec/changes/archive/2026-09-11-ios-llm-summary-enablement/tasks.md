...
- [x] 4.1 Mac上でollamaを起動し、モデルが1つ以上入っていることを確認する
- [x] 4.2 `fvm flutter run -d <iPad simulator>` でシミュレータにアプリを載せる
- [x] 4.3 設定ダイアログを開き、LLMセクションが表示されることを確認する。プロバイダにOllamaを選び、既定のURL `http://localhost:11434` のままモデル一覧が取得できることを確認する。ここで平文HTTPが通るかどうかが確定する
- [x] 4.4 テキストを選択し、横書きモードの選択ツールバーから「解析開始(ネタバレなし)」を実行できることを確認する
- [x] 4.5 縦書きモードで選択範囲内をタップしてメニューを開き、同様に解析を実行できることを確認する
- [x] 4.6 左カラムに履歴タブが現れ、解析結果が一覧に積まれることを確認する
- [x] 4.7 4.3で接続が失敗した場合、`design.md` の該当判断を訂正し、`llmSummary` を `!isIOS` へ戻すかどうかをユーザーに確認する（接続は成功したため差し戻し不要。確認できた範囲を `design.md` に記録する）
