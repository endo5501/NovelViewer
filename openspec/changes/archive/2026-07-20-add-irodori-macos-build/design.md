...
**[上流が macOS を公式サポートしていない]** → `docs/build/` に macos.md が無く、Metal バックエンドで Irodori のオペレータが全て実装されているかはビルド成功だけでは保証されない。ビルド完走とは別に、実機で1文合成する E2E 確認をタスクに含める。

**結果 (タスク 5.1-5.3 で確認済み)**: 実機で初期化・1文合成ともに成功。埋め込み spec が `@builtin` 経由で解決され、Metal バックエンドで未実装オペレータ等の問題は発生しなかった。CPU フォールバックの発動はなく、別 change への切り出しも不要となった。
