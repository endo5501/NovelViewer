...
- LLM 要約のサーフェスを未対応プラットフォームで隠す。設定の LLM セクション、横書き・縦書き両方の選択メニューの解析 2 項目、そしてホバーポップアップの再解析コントロール。保存済み要約の**閲覧**は塞がない（デスクトップで作った成果を iPad で読めるようにするため）
- 更新と同じく、LLM も UI を隠すだけでなく `AnalysisRunner` の入口で拒む。当初はホバーがタッチ端末で発火しないことを理由にポップアップを対象外と考えていたが、**iPadOS はトラックパッド接続時にホバーイベントを配送する**ため、これは誤りだった
- TTS 未対応時は再生トグルのショートカットを登録しない。登録したままだとキー入力を消費して何も起きず、しかもその行は設定一覧から隠れているため、ユーザーには「見えず・変更できず・押しても無反応」のキーになる
- 選択メニューの「辞書に追加」を TTS 未対応プラットフォームで隠す
- 既定ショートカットの主修飾キー判定を「macOS」から「Apple プラットフォーム」に広げ、iOS でも ⌘ を使う

### スコープ外

- **左カラム「解析履歴」タブ**: 解析を起動する経路を持たない読み取り専用パネルで、パネルのコンテキストメニューは右クリックにのみ反応するため、タッチからは開けない。iPad では常に空のタブになるだけであり、Files 経由でデスクトップの `novel_data.db` を持ち込めば閲覧に使える。タブ構成の変更は左カラムを作り替える change D で扱う
...
- `llm-summary-hover-popup`: ホバーポップアップの再解析コントロールも同様に出さない。保存済み要約の閲覧は従来どおり
