...
### D9: LLM も更新と同じ二層構造にする

レビューで、ホバーポップアップの再解析コントロールがどのゲートも通っていないことが分かった。explore 時点では「ホバーはタッチで発火しないから到達不能」と判断していたが、**iPadOS はトラックパッドを接続するとホバーイベントを配送する**。README が前提にしている Magic Keyboard 構成がまさにそれで、デスクトップから持ち込んだ小説フォルダにマークがあれば条件は揃う。

そこで D4 と同じ形にする。

```
UI 層     HoverPopupWidget       trailing: llmSupported ? 再解析ボタン : null
          選択メニュー            ラベル・コールバックを渡さない
入口層    AnalysisRunner.run / runWithScope   先頭で return
```

入口層は無言で戻る。更新チェックが `UpdateSkipped` という結果型を返すのと違い、ここには説明すべきユーザー操作が存在しない — コントロールを出していないのだから、押されること自体が想定外である。

閲覧は塞がない。保存済み要約の表示、履歴タブ、マークの描画はそのまま動く。iPad で禁じるのは新しい解析の**生成**だけで、デスクトップで作った成果の**消費**は歓迎する。

### D10: TTS 未対応時はショートカットを登録しない

`ttsToggle` の binding をマップに残しておくと、キー入力を消費したうえで何も起きない。トグル要求を受け取る `TtsControlsBar` が mount されないためである。しかも change B でその行は設定一覧から隠され、競合判定からも除外されているので、ユーザーからは「見えない・変更できない・押しても無反応」のキーに見える。C で既定が ⌘T に変わることで、iPad の外付けキーボードで実際に押せてしまう組み合わせにもなった。

登録しなければ、キーは他のハンドラに素通しされる。テストもその素通しを確認する形にしてある。
