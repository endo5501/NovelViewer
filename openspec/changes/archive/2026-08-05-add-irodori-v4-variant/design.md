...
**検証済みの範囲:** f16 は Windows/Vulkan (AMD RX 6800)、Windows/CPU、macOS/Metal のいずれでも v3・v4 とも正常に合成できることを確認した。

**未検証の範囲:** q8_0 の破綻を確認したのは AMD RX 6800 / proprietary driver の Vulkan のみである。Metal で q8_0 が同様に壊れるかは未確認 (f16 を採用したため確認する必要がなくなった)。NVIDIA や他ドライバでの挙動も不明。この事象は upstream に報告する価値がある (再現手順が明確で、モデル非依存の可能性が高い)。
