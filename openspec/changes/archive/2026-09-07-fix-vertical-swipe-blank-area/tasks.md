...
- [x] 5d.1 列間の隙間に指を置くとドラッグ全体が無効になる件（anchor が snap しない）のテストを追加し RED を確認する
- [x] 5d.2 anchor 解決を `snapToNearest: true, maxSnapDistance: columnSpacing` にする（タップ経路と同じ規則）
- [x] 5d.3 アニメーション中のスワイプが outgoing ページに飲まれる件のテストを追加し RED を確認する
- [x] 5d.4 outgoing ページにも `onSwipe` を配線する
- [x] 5d.5 「owner 提供の選択がある間は通知するな」の再々指摘は 5b.6 と同じ理由で不採用とする
- [x] 5d.6 spec に上記2件の要件・シナリオを反映する
- [x] 5d.7 境界ページでのスワイプが選択解除を通知することの統合テストを追加する（修正を外すと RED になることを確認）
- [x] 5d.8 design.md を実装の実態に合わせて更新する（決定4の前提誤りの訂正、決定5〜7の追記）
