...
- [x] 8.1 `_paginateLines`で利用可能な高さ（`constraints.maxHeight`）を考慮し、各行が占有する視覚カラム数を`(行の文字数 * 文字高さ / 利用可能高さ).ceil()`で推定するロジックに変更
- [x] 8.2 ページあたりの視覚カラム上限に達するまで行を割り当てるようページ分割ロジックを修正
- [x] 8.3 長い行が複数視覚カラムにまたがるケースのユニットテストを追加

## 9. Issue修正: 縦方向の文字間スペース (Issue 2)

- [x] 9.1 `VerticalTextPage`の`Wrap`の`spacing`を`0.0`に変更
- [x] 9.2 `_buildCharWidget`内でTextStyleの`height`を`1.1`に設定

## 10. Issue修正: VerticalRotatedマッピング網羅 (Issue 3)

- [x] 10.1 `verticalCharMap`に長音・ダッシュ類を追加（`ー`→`丨`, `ｰ`→`丨`, `-`→`丨`, `_`→`丨`, `−`→`丨`, `－`→`丨`）
- [x] 10.2 `verticalCharMap`に波線を追加（`〜`→`丨`, `～`→`丨`）
- [x] 10.3 `verticalCharMap`に矢印の90°回転を追加（`↑`→`→`, `↓`→`←`, `←`→`↑`, `→`→`↓`）
- [x] 10.4 `verticalCharMap`にコロン・セミコロンを追加（`：`→`︓`, `:`→`︓`, `；`→`︔`, `;`→`︔`）
- [x] 10.5 `verticalCharMap`にイコールを追加（`＝`→`॥`, `=`→`॥`）
- [x] 10.6 `verticalCharMap`にスラッシュ・二点リーダー・スペースを追加（`／`→`＼`, `‥`→`︰`, `' '`→`'　'`）
- [x] 10.7 `verticalCharMap`に半角括弧類を追加（`[]`→`﹇﹈`, `{}`→`︷︸`, `<>`→`︿﹀`, `｢｣`→`﹁﹂`, `､`→`︑`）
- [x] 10.8 既存マッピングを修正（`─`→`丨`, `—`→`丨`, `.`→エントリ削除）
- [x] 10.9 追加・修正した全マッピングのユニットテストを更新

## 11. Issue修正: ルビ付き文字の位置ずれ (Issue 4)

- [x] 11.1 `VerticalRubyTextWidget`をRow方式からStack方式に変更（ベーステキストを通常の文字幅で描画し、ルビをオーバーレイ）
- [x] 11.2 ルビテキストを`Positioned`または`Transform.translate`でベーステキスト右側に配置
- [x] 11.3 ルビ付き文字がWrapのカラム幅に影響しないことを確認
