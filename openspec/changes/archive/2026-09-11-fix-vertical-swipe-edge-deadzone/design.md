...
- 横書きモードの操作
- ビューア下端に残る 24pt の死角。ページ番号行は `Column` の 2 番目の子で `GestureDetector` の外側にあるため、同じ構造の死角がここに残る（レビューで実測確認済み）。narrow レイアウトでは物理画面の下端に露出するが、読書中にこの位置から操作することがないため対処しないと判断した。対処が必要になった場合は、ページ番号行を `IgnorePointer` 付きの `Stack` オーバーレイにして `Expanded` を全高にするのが、今回の修正と同じ考え方になる
