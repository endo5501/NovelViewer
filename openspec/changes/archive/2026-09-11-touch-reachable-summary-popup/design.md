...
```
Listener(onPointerDown)            Listener(onPointerUp)
   │ kind がタッチか確認             │ 降りた位置から離れていないか確認
   │ 降りた位置を控える              │ 離れていればタップではない (スクロール等)
   │                                │ global 座標を控え、マイクロタスクを積む
   ▼                                ▼
                 SelectableText.onSelectionChanged
                   cause == tap のとき、キャレットを控える
   ▼
   マイクロタスクで、この指が使えるキャレットからマークを引く
     見つかった → show(word, position, token)
     見つからない → hide()
```

タップ認識器の解決はポインタアップの同期処理の中で完了するため、マイクロタスクはその後に走る。

**指を離しただけではタップと見なさない。** ドラッグの終わりもテキスト上でのポインタアップであり、両者を区別する材料が他にない。降りた位置から `kTouchSlop` を超えて動いていれば解決しない。これを欠くと、指でスクロールして離した瞬間に直前の語の要約が、指が止まった場所に復活する。

**控えたキャレットを使えるのは同じ場所からだけとする。** キャレットが報告されないのには二つの理由があり、一つは同じ場所を再びタップして選択が動かなかった場合、もう一つは二度タップや選択範囲内のタップのように、プラットフォームが別のジェスチャとして解釈した場合である。前者だけが古い答えを引き継いでよい。後者にまで引き継がせると、一つめのタップの語が二つめのタップの場所に出る。

### 決定3b: どちらの文字に触れたかは、キャレットの向きで決める

キャレットは文字と文字の境界に立つので、報告されるオフセットだけでは文字を特定できない。ある語が終わる位置は、次の語が始まる位置と同じ値である。

```
   ア  リ  ス │ ボ  ブ
  0   1   2   3   4   5      ← キャレットのオフセット
              ↑
        「ス」の右半分も「ボ」の左半分も、どちらも 3 を報告する
```

`TextPosition.affinity` がその区別を持っている。`upstream` なら境界の手前の文字、`downstream` なら後ろの文字に触れている。iOS の語端スナップは語の先頭を `downstream`、語の末尾を `upstream` で返すため、どちらの半分をタップしても語の内側の文字に解決する。

**代替案:** オフセットとその一つ手前の両方でマークを引く。却下した。隣り合う二つの解析済み語句のうち前者の末尾をタップすると、先に後者が引かれて隣の要約が開く。マークの直後の未マーク文字をタップしても前の語が開く。どちらも向きを見れば起きない。
...
**[語端スナップによるオフセットのずれ] → 対策:** iOS のタッチタップは `selectWordEdge` を通るため、報告されるオフセットが語の端に寄る。決定3b のとおり `affinity` で触れた文字を特定する。隣り合うマーク、マークの直後の未マーク文字、語末へのスナップの三つをウィジェットテストで確かめる。

**[マイクロタスクのタイミングが認識器の解決より先になる] → 対策:** タップ認識器が二度タップの判定で遅延を持つ場合、マイクロタスクのほうが先に走る可能性がある。ウィジェットテストで確認した結果、単独のタップではアリーナの掃引が同期的に完了するため先行しない。二度タップの二打目はそもそも `cause: tap` を報告しないので、決定3 の「同じ場所からだけ引き継ぐ」規則で扱う。
...
**[ポインタの離脱がウィジェットの破棄中に届く] → 対策:** マークの上にポインタを置いたままビューアが木から外れると、`MouseTracker` はその瞬間に退出を発火する。`ref` はそのとき使えない。ポップアップの通知子を構築時に一度取り出して保持し、退出がいつ来てもポップアップを畳めるようにする。
