...
- [x] 10.1 `path_provider` パッケージを pubspec.yaml に追加
- [x] 10.2 デフォルトディレクトリパス（`~/Documents/NovelViewer/`）を解決・自動作成するサービスを実装
- [x] 10.3 アプリ起動時にデフォルトディレクトリを `currentDirectoryProvider` の初期値として設定
- [x] 10.4 デフォルトディレクトリ関連のテストを作成

## 11. ダウンロードダイアログから出力先選択UIを削除

- [x] 11.1 ダウンロードダイアログから出力先ディレクトリ選択行（フォルダアイコン + パス表示）を削除
- [x] 11.2 ダウンロード先をデフォルトディレクトリに固定するようにプロバイダを変更
- [x] 11.3 ダウンロードダイアログのテストを更新（出力先選択関連のテストを削除・修正）

## 12. ファイルブラウザからディレクトリ選択UIを削除

- [x] 12.1 ファイルブラウザの「フォルダを開く」ボタン（ディレクトリピッカー）を削除
- [x] 12.2 `file_picker` パッケージへの依存が不要であれば pubspec.yaml から削除
- [x] 12.3 ファイルブラウザ関連のテストを更新（ディレクトリ選択関連のテスト修正）

## 13. Bug fix

- [x] 13.1 ダウンロードを実施すると、次のエラーが発生して失敗する:`エラー：ClientException with SocketException: Failed host lookup: 'ncode.syosetu.com' (OS Error: nodename nor servname provided, or not known, errno = 8), uri=https://ncode.syosetu.com/n8281jr/`
