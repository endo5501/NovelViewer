...
- **コレクションはライブラリ直下**: ダイアログは既存コレクションのパスを `join(libraryPath, folderName)` で再構築するため、コレクションフォルダを整理用サブフォルダへ移動すると追記先パスが解決できない。MVP ではコレクションはライブラリ直下に置く前提とする（専用サイトの refresh が parentPath を呼び出し側から受け取るのと同様、将来はファイルブラウザのパス解決へ寄せる）。
- **コレクションの一括 refresh は不可**: コレクションは複数ソースの集合のため、novel の refresh 経路は siteType='web' を検出して拒否する（D9 同様、記事ごとに再ダウンロードする）。
