...
デスクトップは既定 1280pt、保存サイズの復元時も 800pt を下回らないため、通常の使い方で narrow になることはない。ただし読者がウィンドウを 800pt 未満まで縮めれば narrow になる——ネイティブウィンドウに最小サイズは課されていないため——これは意図した挙動であり、「デスクトップを 800pt まで縮めて検索を開くと本文が 248pt になる」既存の穴を同じ仕組みで塞ぐ。
