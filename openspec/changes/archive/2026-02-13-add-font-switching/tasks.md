...
- [x] 7.1 `_paginateLines()`のTextPainter実測化に対するテストを作成。フォントサイズを大きく変更（例: 32.0）した際に、縦書き表示で`VerticalTextPage`の`Wrap`がオーバーフローせず正しくページ分割されることを検証するウィジェットテストを書く
- [x] 7.2 `_paginateLines()`の`charHeight`と`columnWidth`の計算を、`fontSize`からの推定値ではなく`TextPainter`で代表文字（'あ'）を実測した値に置き換える。`charHeight = painter.height`、`columnWidth = painter.width + _kRunSpacing`とする
- [x] 7.3 フォント種別変更時（例: `hiraginoMincho`）に縦書きページネーションが正しく再計算されることを確認するウィジェットテストを作成
- [x] 7.4 既存の縦書き関連テストが全て通ることを`fvm flutter test`で確認
