...
**Reason**: 左カラムは幅に依らず常に `Drawer` に置かれることになり、「narrow のときだけ drawer に移す」という条件が成立しなくなった。また「drawer は wide の左カラムと同じ幅」という規定が、ファイル名が省略される原因そのものだった。

**Migration**: 左カラムについては新しい要件 "The left column always lives in a drawer" が、右カラムについては "The narrow layout moves the search results into an end drawer" が置き換える。右カラムの扱い（narrow では `endDrawer`、wide では列）は変更されていない。

### Requirement: The narrow layout's drawers keep their contents clear of the system bars
**Reason**: インセットが narrow レイアウト限定の措置ではなくなった。ファイルブラウザの drawer はすべての表示幅に存在するため、インセットも常時適用される。

**Migration**: 新しい要件 "The drawers keep their contents clear of the system bars" が置き換える。インセットの対象・範囲・drawer 表面が余白を覆うことは変更されていない。
