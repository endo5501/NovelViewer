...
- [x] 7.1 macOS で `fvm flutter run -d macos` を実行し、起動時に `Drawer` が開いて前回のエピソードが選択済みで中央に表示されること、長いファイル名が省略されずに読めること、`Tab` と Esc が期待どおり動くことを目視で確認する
- [x] 7.2 起動時に `Drawer` が開くまでの待ち時間が許容範囲か確認する。体感できるほど遅い場合は design の Risks に記載した代替案（プレースホルダ表示、または D2 却下案の再検討）を検討対象として記録する
...
- [x] 8.6 `/opsx:sync` で delta spec を本体へ同期した後、`openspec/specs/adaptive-shell-layout/spec.md` と `openspec/specs/three-column-layout/spec.md` の `## Purpose` を手で更新する（delta spec の Purpose は同期時に無視されるため。design D7）
