...
### リンク入力源を抽象化し、`app_links` を触るのは 1 箇所だけにする

`Stream<Uri>` を返す薄い入力源の抽象を置き、`app_links` を包むのはその名前付きコンストラクタ 1 つだけにする。利用側はこの抽象にだけ依存させ、テストでは fake のストリームに差し替える。`shared/platform/platform_capabilities.dart` が `dart:io` を知らない形で書かれているのと同じ流儀で、「プラットフォーム依存の読み取りは 1 箇所に閉じる」を踏襲する。

そのコンストラクタを呼ぶのは `main.dart` の 1 行だけである（理由は後述の「入力源の既定は…」）。プロバイダ自身はプラットフォームに触れない。
...
- **初回リンクの取得失敗が未処理例外になる** → 初回リンクはアプリ起動時に要求されるが、購読は最初の画面が組み上がるまで始まらない。その間に失敗するとハンドラが未装着で、Dart はこれを未捕捉エラーとして報告する。入力源のコンストラクタで即座に受け止め、「リンクでは起動していない」として扱う。
