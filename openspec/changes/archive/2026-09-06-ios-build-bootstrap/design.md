...
これら 2 箇所に加え、一般タブのショートカット一覧から TTS 切り替え行も外す。押しても何も起きないキーを再割り当てできる状態は、機能が無いことより「あるのに壊れている」と読めるため質が悪い。

行を隠すだけでは不十分で、`rebind` の衝突判定からも外す必要がある。判定は保存されている全アクションを対象にするため、隠した TTS のバインドが他アクションへの割り当てを「重複」として拒否し、しかもユーザーにはそれを解放する手段が無い、という袋小路が生じる。利用できないアクションは衝突相手として数えない。バインド自体は保存に残すため、そのプラットフォームで TTS が使えるようになれば元の割り当てが戻る。
...
**[debug ビルドは実機のホーム画面から起動できない]** → iOS 14 以降、debug ビルドが必要とする JIT は Flutter ツール配下でしか許可されない。ホーム画面から起動するとその旨のメッセージだけが表示され、アプリは立ち上がらない。iPad を Mac から切り離して常用するという目的からすると、配布形態は `--release` ビルドのインストールになる。README のビルド手順もそれを前提とし、debug の制約自体も記載して、メッセージを見たときに異常と誤認されないようにする。
