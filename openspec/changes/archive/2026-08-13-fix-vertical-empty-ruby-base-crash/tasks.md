...
- [x] 4.1 `fvm flutter build windows` でビルドし、`hameln_419738/07_07.txt` を縦書きで開いて本文が表示されることを確認する（ユーザー確認済み）
- [x] 4.2 同様に `hameln_419738/09_09.txt`（空 base ルビ 4 件）を縦書きで開いて本文が表示されることを確認する（ユーザー確認済み）
- [x] 4.3 同じ 2 ファイルを横書きで開き、修正前と表示が変わっていないことを確認する（ユーザー確認済み）
- [x] 4.4 空 base ルビを含むページで TTS 再生を行い、ハイライト位置がずれていないことを確認する（ユーザー確認済み）

## 5. 最終確認

- [x] 5.1 code-reviewスキルを使用してコードレビューを実施（指摘2件。行頭禁則マスクは 6.x で修正、本文0文字の列は既知の制限として design.md に記録）
- [x] 5.2 codexスキルを使用して現在開発中のコードレビューを実施（行頭禁則マスクを同様に指摘。6.x で修正）

## 6. レビュー指摘対応: 空ルビが行頭禁則判定を隠す問題

- [x] 6.1 空 base ルビの直後に行頭禁則文字が来る場合、平文のみの場合と同じ列構造になることを検証する失敗テストを追加する
- [x] 6.2 連続する空 base ルビでも同様であることを検証する失敗テストを追加する
- [x] 6.3 行末禁則側は影響を受けないことを固定する回帰ガードテストを追加する
- [x] 6.4 `_lineHeadCharFrom` / `_lineEndCharOf` を導入し、`charCount == 0` のエントリを読み飛ばして可視文字で禁則判定するよう `splitWithKinsoku` を修正する
- [x] 6.5 `fvm flutter analyze` と `fvm flutter test` を再実行して回帰がないことを確認する
