...
`{LibraryParentDir}/cache/embeddings/` ディレクトリに配置する。既存の`voices/`ディレクトリとは分離し、キャッシュはいつでも削除可能な揮発性データとして扱う。
...
- **[リスク] モデル変更時のキャッシュ無効化** → 異なるモデル（0.6B vs 1.7B）ではエンコーダの出力が異なる可能性がある。初期実装ではモデル変更時のキャッシュ無効化は行わない（エンベディング次元は同一のため実害は低い）。将来必要になればキャッシュキーにモデル名を含める拡張が可能。
- **[トレードオフ] エンベディングの次元検証** → `synthesize_with_embedding`呼び出し時にエンベディングサイズの検証（1024要素であること）を行い、不正なキャッシュの使用を防止する。
