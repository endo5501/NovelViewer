...
### D8: モードが切り替わるときの Drawer は Flutter に任せる（実装時に訂正）

当初は「Drawer を開いたまま横向きにすると `Scaffold` から `drawer` が消える一方で `Navigator` にルートが残りうるので、明示的に閉じる」と書いていた。実装時にテストで確かめたところ、**その前提が誤っていた**。

`Scaffold` の Drawer は `Navigator` のルートではなく `Scaffold` 自身の `build` が組む `DrawerController` である。`drawer` が `null` になれば `DrawerController` ごと破棄され、スクリムも残らない。したがって明示的なクローズ処理は不要である。

さらに重要な点として、この破棄では `onEndDrawerChanged(false)` が**発火しない**。もし発火していれば D6 の書き戻しが働き、「横向きにしたら検索セッションが消える」というバグになっていた。実際には provider は true のまま残り、wide の行内右カラムとして検索結果が表示され続ける——これが望ましい挙動である。

コードは足さず、この 2 点をウィジェットテストで固定する。
