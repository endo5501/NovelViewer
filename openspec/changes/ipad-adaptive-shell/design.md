...
3. ウィジェットテストのほぼ全てが Flutter 既定の 800×600 論理 px で走っている。画面サイズを設定しているのは設定ダイアログ関連の 3 本（いずれも 1200×900）だけで、ホーム画面を組み立てるテストには 1 つも無い。
...
- デスクトップの**通常の**レイアウトを変えること。既定 1280pt でも、最小復元幅 800pt でも 3 カラムのままである。ただし読者が意図的に 800pt 未満まで縮めた場合は narrow になる（D2 参照。当初「あり得ない」と書いていたのは誤りだった）。
...
800pt は `kMinimumWindowSize = Size(800, 600)`（`window_size_resolver.dart:11`）と一致する。デスクトップ版は保存したウィンドウサイズを復元する際にこの幅まで引き上げており、その理由は「保存された細切れのウィンドウが使い物にならない 3 カラムとして復活しないように」である。すなわち **デスクトップ側が既に「3 カラムが収まる最小幅」と判断している数字**であり、恣意的な選択ではない。

**当初ここに「デスクトップのウィンドウは決して narrow にならない」と書いていたが、これは誤りだった**（レビューで指摘）。`kMinimumWindowSize` は復元時のクランプにしか使われておらず、`setMinimumSize` 相当の呼び出しはリポジトリのどこにも無い。読者はデスクトップのウィンドウを 800pt 未満にドラッグでき、そのとき narrow レイアウトになる。

これは受け入れる。むしろ望ましい——探索時に見つけた「デスクトップを 800pt まで縮めて検索を開くと本文が 248pt になる」既存の穴が、同じ仕組みで塞がるからである。ただし副作用として、狭いデスクトップウィンドウでは Tab のペイン切替（D9）とカラム切替ボタン（D11）が消える。いずれも「Drawer の中にあるものへの操作」なので一貫してはいるが、デスクトップの挙動は「検索ボタンが増えるだけ」ではなくなった。
...
**副次的な利点:** テストの既定ビューポート幅 800pt は `800 < 800` が偽なので wide 側に落ちる。既存のウィジェットテストは 1 行も変更せずに現在と同じレイアウトをレンダリングする（画面サイズを設定している 3 本も 1200pt なので同じく wide）。Material の medium 境界である 840pt を採ると、既存のホーム画面テスト群がすべて narrow でレンダリングされ、各テストに画面サイズの設定を足して回る作業が発生していた。
...
例外は意図的に置いた 4 本だけである。実ビューポートでの裏取り 1 本（iPad mini 縦 744pt）と、しきい値の横断そのものを検証する 3 本——これらは幅の変化が主題なので、しきい値を動かしたのでは検証にならない。
...
### D7b: 契機は「どのファイルが開いているか」ではなく「開く操作が起きたこと」（レビューで 2 度修正）

D7 では `selectedFileProvider` を listen すると書いたが、レビューで**両方向に外れている**ことが分かった。

```
発火しすぎ  フォルダに入るとき file_browser_panel が selectedFileProvider.clear()
            を呼ぶ → フォルダを開いたそのタップで Drawer が閉じる。
            読者はまだ選んでいる最中である。

発火しなさすぎ  ファイル一覧は directoryContentsProvider にキャッシュされた
            FileEntry をそのまま渡し、FileEntry は == を実装していない。
            今読んでいる話をもう一度タップすると Riverpod は同値と見なして
            誰にも通知せず、Drawer は開いたまま残る。
```

どちらも「開いているファイルの同一性」を契機にしたことに由来する。実際に必要なのは**開く操作が起きたこと**なので、`fileOpenRequestProvider`（`selectFile` のたびに増える整数）を追加し、シェルはそれを listen する。同じファイルの再選択でも増え、`clear()` では増えないので、両方向が同時に直る。

`FileEntry` に `==` を与える案は却下した。ブックマークタブは毎回 `FileEntry` を作り直しており、同じブックマークを 2 度タップしたときに現在は再ジャンプする。値等価にするとそれが黙って効かなくなる。

この形は TTS の `ttsToggleRequestProvider` / `ttsStopRequestProvider` と同じ「リクエストカウンタ」の書き方であり、既存の語彙に収まる。

残る競合として、起動時の読書位置復元（`readingProgressStartupProvider`）が非同期にファイルを開くため、その完了前に読者が Drawer を開いていると閉じられる。復元は起動直後の一度きりで、影響は「≡ で開き直す」に留まるため、区別する仕組みは入れずに受け入れる。

### D8: しきい値をまたぐときは、閉じるのではなく narrow 進入時に突き合わせる（実装時に 2 度訂正）

当初は「Drawer を開いたまま横向きにすると `Scaffold` から `drawer` が消える一方で `Navigator` にルートが残りうるので、明示的に閉じる」と書いていた。**前提が誤っていた。** `Scaffold` の Drawer は `Navigator` のルートではなく `Scaffold` 自身の `build` が組む `DrawerController` であり、`drawer` が `null` になれば丸ごと破棄され、スクリムも残らない。しかもこの破棄では `onEndDrawerChanged(false)` が発火しないため、D6 の書き戻しが誤作動して「横向きにしたら検索が消える」ことにもならない。

そこで「コードは足さない」と結論したが、これも不十分だった。narrow → wide の片方向しか検証していなかったためで、レビューで **逆向きに 2 つの穴**が指摘された。

```
① wide で検索を開く → narrow へ
   provider は true のまま変化しないので ref.listen は発火せず、
   新しく作られた endDrawer は閉じたまま。
   アプリは「検索は表示中」と思っているのに画面のどこにも無い。

② narrow で endDrawer を開く → wide へ（破棄。ただし Scaffold は
   _endDrawerOpened = true を保持したまま）→ 検索を閉じる → narrow へ
   Scaffold は保持したフラグを新しい DrawerController に渡すので、
   閉じたはずの検索が開いた状態で復活する。
```

どちらも「provider の変化」を契機にしているせいで、**変化しない側**を取りこぼしている。したがって必要なのはクローズ処理ではなく、**narrow に入った時点で provider と Drawer を一度突き合わせること**である。`_lastLayout` でしきい値の横断を検出し、post-frame で `_syncEndDrawer(ref.read(rightColumnVisibleProvider))` を 1 回呼ぶ。開いているべきなら開き、閉じているべきなら閉じる。

②の経路では復活した Drawer が一瞬見えてから閉じるが、`Scaffold` の内部状態に手を入れずに済ませる代償として受け入れる。
...
- `initState` の post-frame にある `_fileBrowserPaneFocus.requestFocus()` は narrow では行わない。`MediaQuery` は `initState` では読めないが、post-frame コールバックの時点では `context` が使えるのでそこで判定する。

  なお実装時に確認したところ、**この要求は現状でも実害がない**——未アタッチの `FocusNode` は `_manager` を持たないため要求が黙って捨てられるからである。したがってこのガードを外してもテストは落ちない。それでも書くのは、フレームワークの内部挙動に依存して正しいのではなく、構造として正しくするためである。バインディングを登録しない側（Tab が素通りすること）は観測可能で、テストが判別する。
...
### D14: 解析履歴タブの削除は「常に空」ではなく「到達経路の放棄」である（実装時に訂正）

提案時、このタブは iPad では常に空だと書いた。**それは誤りだった。** `LlmSummaryHistoryNotifier` は現在のフォルダの `llmSummaryRepositoryProvider` を読む——すなわち小説フォルダ内の `novel_data.db` であり、デスクトップで解析した小説フォルダを持ち込めば中身が入る。変更 C でホバーポップアップについて犯したのと同じ種類の見落としである。

iPad で保存済み要約に到達する経路は 3 つしかない。

```
解析履歴タブ           → 本変更で削除
ホバーポップアップ      → MouseRegion.onHover。トラックパッド接続時のみ発火
コンテキストメニュー解析 → 変更 C で非表示
```

したがってタブを消すと、**素の iPad では持ち込んだ解析結果に到達できなくなる**。これは機能の後退であり、変更 C が README に書いた「デスクトップで作成した解析結果は iPad の『解析履歴』から閲覧できます」と矛盾する。

利用者にこの事実を提示した上で、**削除を維持する**判断を得た。iPad 版は小説ビューアに専念するという方針（変更 C）の延長であり、要約の閲覧はトラックパッド接続時のホバーに限られる。README の当該記述はその通りに訂正した。

**却下案:** 履歴が空でないときだけタブを出す。両立はするが、タブ構成が開いている小説フォルダによって変わることになり、`TabController` を作り直す必要が生じる。「この小説にはタブがあってあの小説には無い」という説明しづらい UI にもなる。
