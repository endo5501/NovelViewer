...
### 8.8 `/opsx:verify` で検出したカバレッジ欠落の解消

- [x] 仕様「初期位置は先頭」に対応する自動テストが無かった（`ttsEditCursorIndexProvider` / `ttsEditPlayingProvider` をテストが一切参照していない）→ `test/features/tts/providers/tts_edit_providers_test.dart` を新規作成
- [x] 仕様「進行方向と逆側へは引き戻さない」に対応する自動テストが無かった。既存テストは「後退移動で上へ追う」ケースしか見ておらず、`alignmentPolicy` を `explicit` に落としても検出できなかった → ユーザーが再生位置を追い越してスクロールした状態で前進させるテストを追加。`explicit` に変えると当該テストを含む3件が落ちることを確認済み
