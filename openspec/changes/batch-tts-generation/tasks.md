...
- [x] 3.1 `TtsAudioPlayer` 抽象に `pause()` メソッドを追加、`JustAudioPlayer` に実装
- [x] 3.2 `TtsStoredPlayerController` クラスを作成（DB→WAV BLOB取得→一時ファイル書き出し→再生の逐次パイプライン）
- [x] 3.3 次セグメントの先読み（DB BLOB読み込み＋一時ファイル書き出し）を実装
- [x] 3.4 一時停止/再開機能を実装
- [x] 3.5 停止機能を実装（位置リセット、ハイライトクリア、一時ファイル削除）
- [x] 3.6 テキスト位置指定再生を実装（text_offsetからセグメント特定→そのセグメントから再生開始）
- [x] 3.7 ハイライト連動を実装（既存の `ttsHighlightRangeProvider` を使用）
- [x] 3.8 自動ページ送りとの連携を確認（既存メカニズムの再利用）
