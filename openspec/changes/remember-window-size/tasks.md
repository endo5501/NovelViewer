...
- [x] 1.2 `fvm flutter build windows` が通ることを確認し、macOS / Linux のビルド設定を壊していないことを確認する
...
- [x] 5.4 `windowManager.addListener(WindowStateRecorder(...))` を登録する。`onWindowClose` が発火するために追加設定（`setPreventClose` 等）が必要かを確認し、必要な場合はフラッシュ後に確実にウィンドウが閉じることを実機で検証する
