...
- [ ] 8.1 `_noServiceMessage` を `findLlmConfigProblem` に基づく分岐に置き換える
- [ ] 8.2 オンデバイスの可用性メッセージを既存のまま残す
- [ ] 8.3 表示経路が既存の `_snack` のままであることを確認する（`showFailureSnackBar` は使わない）
- [ ] 8.4 7章のテストがすべて通ることを確認する
- [ ] 8.5 コミットする

## 9. 3言語の文言確認

- [ ] 9.1 日本語・英語・中国語の3ファイルにキーが揃っていることを確認する
- [ ] 9.2 既存の `llmAnalysis_noLlmConfigured` が削除されずプロバイダ未設定用として残っていることを確認する
