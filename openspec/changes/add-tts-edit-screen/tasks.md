...
- [x] 6.1 `TtsEditDialog`ウィジェットを作成: ダイアログ外枠、タイトル、上部ツールバー（全再生/全生成/全消去ボタン）
- [x] 6.2 `TtsEditSegmentRow`ウィジェットを作成: 状態アイコン、テキストフィールド、リファレンス音声ドロップダウン、メモフィールド、再生/再生成/リセットボタン
- [x] 6.3 リファレンス音声セレクターを実装: 「設定値」「なし」+voicesフォルダ内ファイル一覧、ファイル不在時の「無し」表示
- [x] 6.4 ダイアログ閉鎖時のクリーンアップ実装: TtsIsolateの破棄
...
- [x] 8.1 混在生成状態（一部生成済み・一部未生成）での再生のテストを作成
- [x] 8.2 オンデマンド生成（未生成セグメント到達時にDB上のtextで生成→再生）のテストを作成
- [x] 8.3 セグメント単位のref_wav_pathを使用した生成のテストを作成
- [x] 8.4 `TtsStreamingController`を修正: 再生ループでセグメントごとにaudio_dataの有無を確認し、未生成の場合はオンデマンドで生成してから再生する
