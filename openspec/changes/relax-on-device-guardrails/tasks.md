...
- [ ] 4.1 `test/features/llm_summary/data/llm_parsed_response_test.dart` に、コードフェンスで囲まれ `facts` が文字列であるオブジェクトが読めることのテストを追加する(既存のカバレッジがあれば確認のみ)
- [ ] 4.2 同テストに、コードフェンスで囲まれ `facts` が文字列配列であるオブジェクトが読めることのテストを追加する
- [ ] 4.3 同テストに、閉じていないJSONが生テキストとして扱われ解析が止まらないことのテストを追加する
- [ ] 4.4 パイプライン側の変更が不要であることを確認する。必要になった場合は理由を design.md に追記する
