...
- 選択の起点（anchor）を、pan が受理された位置ではなく **ポインタが降りた位置** から解決する。タッチの slop は縦書きの 1〜2 文字分に相当するため、現状は押した文字より後ろから選択が始まる（実測: 'あ' の中心から下方向へドラッグすると選択結果は 'いうえ'）。あわせて、余白起点のドラッグが slop 内の文字に吸い付いてしまう問題も解消される。
