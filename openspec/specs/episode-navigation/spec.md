...
同一ディレクトリ内のテキストファイル並びを、横書き／縦書きビューア共通の「次話／前話」遷移として扱う。隣接ファイルの導出、開始位置ヒント（冒頭／末尾）の受け渡し、選択と intent の同期更新を Riverpod provider 群で提供し、各ビューアはこの capability の Provider のみを参照する。
