...
ローカルでリリース手順（`pubspec.yaml` のバージョン更新・commit・タグ付け・push）を一括実行し、git タグと `pubspec.yaml` の `version` の不整合や不正なリリース状態を push 前に防ぐためのスクリプト群を提供する。PowerShell（`scripts/release.ps1`）と bash（`scripts/release.sh`）の双方を提供し、事前検証に失敗した場合はリポジトリに一切変更を加えずに失敗させる。
## Requirements
