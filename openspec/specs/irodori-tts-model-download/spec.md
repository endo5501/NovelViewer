...
Irodori-TTS の実行に必要なモデル資産を Hugging Face からダウンロードする `IrodoriModelDownloadService` を提供する。資産は variant (`v3` / `v4`) ごとに 1 本の f16 GGUF であり、model spec / model_config / tokenizer / codec をすべて GGUF 内に埋め込むため兄弟ディレクトリを必要としない。variant ごとに独立した進捗表示・ダウンロード済み判定・キャンセル・失敗時の再試行 (完了済みファイルの skip) をサポートし、variant ディレクトリに GGUF が1本だけ残ることを保証する。あわせて旧形式 (safetensors 3ディレクトリ構成) の資産を検出し、ユーザの明示的な操作で後始末できるようにする。
