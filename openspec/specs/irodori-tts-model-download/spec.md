...
Irodori-TTS の実行に必要な4資産 (600M-v3-VoiceDesign モデル / llm-jp トークナイザ / Semantic-DACVAE codec) を Hugging Face から一括ダウンロードする `IrodoriModelDownloadService` を提供する。audio.cpp の model spec が要求する相対配置を保証し、進捗表示・ダウンロード済み判定・キャンセル・失敗時の再試行 (完了済みファイルの skip) をサポートする。
