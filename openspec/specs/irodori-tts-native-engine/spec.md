...
endo5501/audio.cpp フォークを submodule として取り込み、Irodori-TTS (variant ごとの単一 GGUF による v3 / v4) のネイティブ推論を行う共有ライブラリ `audiocpp_ffi` (Windows: Vulkan / macOS: Metal) をビルドし、C API・Dart FFI バインディング・`IrodoriTtsEngine` ラッパーを通じて素TTS / ボイスクローン / caption 指定 / 両立の4形態の合成と安全な abort 機構を提供する。あわせて、同一構成から性能測定用の `audiocpp_cli` をビルドする (配布物には含めない)。
...
尺補正の有効化は、Dart の各層を貫いて運ぶパラメータとしてではなく、**caption を C API に渡す唯一の地点で「caption の有無」と「ロード済みモデルが補正を必要とするか」から導出**しなければならない (MUST)。
後者は variant ごとに決まるモデルの性質なので、**合成リクエストごとではなくモデルロード時に**与えなければならない (SHALL)。

**補正オプションを 1 つも渡さない合成は、従来の合成エントリポイントを使わなければならない (SHALL)。**
オプション付きエントリポイントを持たない古いネイティブライブラリに対しても、
オプションを必要としない経路が動作し続けるためである。
...
- **THEN** 従来の合成エントリポイントが呼ばれ、従来と同じ出力になる

#### Scenario: 補正を必要としないモデルでは caption があってもオプションが付かない
- **WHEN** 補正なしでロードしたモデルに caption を指定して合成を呼ぶ
- **THEN** caption は渡るが補正オプションは付かず、従来の合成エントリポイントが呼ばれる
