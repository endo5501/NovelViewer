...
ツールチップの起動はホバーに限られなければならない（SHALL）。`Tooltip` が既定で持つタッチ長押しによる起動は無効化しなければならない（SHALL）。フォルダタイルの長押しはコンテキストメニューを開く操作であり、`Tooltip` は自身の長押しレコグナイザをより内側で調停に登録するため、無効化しなければタイトル文字の上でメニューが開かなくなるためである。
...
#### Scenario: タイトル文字を長押ししてもツールチップは出ない
- **WHEN** ユーザーがフォルダタイルのタイトル文字の上を長押しする
