...
Two things shape everything here. The model's context window is a fraction of a server model's and holds the prompt and the response together, which is why the chunk budget became a property of the client rather than a constant of the pipeline. And the model can refuse text: a novel carrying violent or sexual description is refused by its safety guardrails, which is not a defect and cannot be retried away, so a refusal is isolated to the file it happened in.
