...
ライブラリ内の小説を一意に識別する `novel_id` を、ネスト配置（整理フォルダ配下など）に依存せず一貫して解決するための単一の共有規則を提供する。novel_id は **登録済み小説フォルダの葉名（`novels.folder_name`）** とし、ブックマーク・読書進捗・小説削除を含む novel_id を必要とする全機能が同一の解決規則を消費することで、保存キーと参照キーの不一致による孤児データや復元失敗を防ぐ。`resolveNovelId` 関数と、現在ディレクトリに対する `currentNovelIdProvider`（非同期）を公開する。
