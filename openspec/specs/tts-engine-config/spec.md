...
Provide a typed, sealed `TtsEngineConfig` that carries engine-specific synthesis parameters (Qwen3 / Piper / Irodori) so that callers obtain a single object from a Riverpod-aware factory rather than assembling parameters inline. This eliminates engine-specific `if/else` branches across the TTS UI surface and keeps the per-engine field set exhaustive at the type level.
