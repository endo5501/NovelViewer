...
GitHub Releases を用いたアプリ内更新チェック基盤。起動時のバックグラウンド確認と手動確認、24 時間のレート制御とスヌーズ（後で）による問い合わせ頻度の抑制、実行中バイナリからの配布形態検出（インストーラ版 / ポータブル版 (ZIP)）、インストーラ版での Level 2 自動更新フロー（ダウンロード→検証→インストーラ起動）、ポータブル版での通知＋リリースページのブラウザ起動フロー、および更新が利用可能な場合の AppBar 更新バッジまでを扱う。アプリ自身を更新する手段を持たないプラットフォームでは、チェック自体を一切行わない。
## Requirements
