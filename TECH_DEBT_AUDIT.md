...
1. **最大のテーマは「静かな失敗」**: TTS合成失敗がエラー表示ゼロで `completed` 扱いになり（F101）、ダウンロードは目次2ページ目以降の取得失敗を握り潰して「完了」と報告し（F102）、サイト改修で空になったエピソードを保存して永久キャッシュする（F105）。自動更新の失敗も空catchで消える（F140）。エラーは起きているのにUIにもログにも届かない構造が複数機能を貫通している。 — ✅ このテーマは全面解決: TTS(F101/F112)・DL(F102/F103/F105)に続き、最後の燃え残りだった自動更新経路の空catch×6(F140)と起動時APIキー移行のdebugPrint×2(F139)も fix-update-path-diagnosability でAppLoggerへ集約（異常=WARNING/想定内フォールバック=FINE）。`lib`内のdebugPrintはAppLogger実装自身のみに。
...
7. **テストは量より分布が問題**: 失敗系（failedCount検証ゼロ）、DBマイグレーション（v3→v4カバレッジゼロ）、isolate/FFI境界、スクレイパーの実HTMLフィクスチャがいずれも空白（F122/F129/F158）。一方で死んだコードに対するテストが2箇所で維持されている（F133/F146）。 — 🟡 DB周りは解決: harden-db-test-schema-fidelity で v3→v4 移行カバレッジ(F129)と本番DDLのテスト手書きドリフト(F130)を解消（本番`_onCreate`を単一の正に抽出＋共有フィクスチャ＋スキーマ一致ガード）。残るは失敗系の網羅(F122)・isolate/FFI境界(F158)。
...
| F111 | 並行性 | lib/features/tts/data/tts_isolate.dart:140-145,231-238 | ~~High~~→**Critical** | M | `abort()`はメインisolateから`_ctxAddress`経由でnativeを直接叩くが、モデル再ロード時に旧ctxがfreeされた後も`ModelLoadedResponse`到着までアドレスがstale。ロード中（数秒窓）のstopで解放済みポインタへのFFI呼び出し＝use-after-free | ✅ 対応済み(fix-tts-abort-use-after-free): spikeで`qwen3_tts_abort`が解放済みctxへの**UAF write**（read より重い）と確定しCritical格上げ（Open Q6成立）。対症(`_ctxAddress=null`)ではなく根治を採用——abortフラグをctxから独立した`qwen3_tts_abort_handle`(atomic bool)へ分離し、`TtsIsolate`がセッション寿命で所有（spawn生成・再ロードでアドレス不変・worker終了後に解放）。`qwen3_tts_init`がhandle引数を取り`ModelLoadedResponse.ctxAddress`撤去。レビュー指摘2件（force-kill経路のUAF再発→timeout時free回避、spawn失敗時リーク→try/catch）も修正。Windows実機で再ロード窓の停止を検証。残: queued synthesizeの無条件resetAbortがstop/dispose abortを打ち消す既存レース（UAF非該当）は別タスク化 |
...
| F118 | 一貫性 | kakuyomu_site.dart:30-116 vs narou_site.dart:51-127, hameln_site.dart:72-126 + download_service.dart:139-168 | Medium | M | 防御の非対称: Kakuyomuのみ構造ドリフトで説明的throw、他は黙ってtitle空/episodes 0件を返す。DownloadService側に「何もパースできなかった」ガードがなく空フォルダ＋「完了」になる | ✅ 対応済み(harden-scraper-parse-contract): アダプタ内throw化ではなく**DownloadService集約の下流ガード**を採用。`parseIndex`直後・`createNovelDirectory`の前に`episodes.isEmpty && bodyContent == null`なら新設`EmptyIndexException`を送出し、空フォルダを残さず呼び出し側へ伝播（目次1ページ目失敗と同じerror扱い）。Aozora/短編は`bodyContent`を立てるため除外。providerは専用メッセージへマッピング。既存specの「episodeCount=0で完了」シナリオを差し替え。Narou/Hamelnのアダプタ内throw化はNon-Goal |
| F119 | セキュリティ | lib/features/text_download/data/sites/kakuyomu_site.dart:25-27 | Medium | S | `url.host.contains('kakuyomu.jp')`は`kakuyomu.jp.evil.com`にもマッチ。他3アダプタは厳密ホスト集合。schemeチェックはどこにもない | ✅ 対応済み(harden-scraper-parse-contract): `KakuyomuSite.canHandle`を厳密ホスト集合`{'kakuyomu.jp','www.kakuyomu.jp'}`＋`/works/<id>`パスチェックへ（他3アダプタとパリティ化）。スキーム検証は`NovelSiteRegistry.findSite`に集約し非webスキーム(javascript:/file:/ftp:等)を一括拒否。httpは受理しつつ全アダプタの`normalizeUrl`でhttps強制（aozoraに追加、他3つは既存）＝fetchと保存URLは常にhttps。これによりレビュー指摘の「http青空文庫リンクの新規入力/更新が弾かれる」回帰を回避しつつ平文transportを排除 |
| F120 | 一貫性 | download_service.dart:59-61,70 vs hameln_site.dart:17-18,44-51 | Medium | S | UA方針が分裂: デフォルトはChrome偽装、HamelnのみCloudflare 403回避で正直UA上書き。上書きが効くのはヘッダspread順という暗黙契約 | ✅ 対応済み(harden-scraper-parse-contract): UA決定を`NovelSite.requestHeaders`を唯一の源とする契約としてspec化し、`siteHeaders`が既定UAを上書きする優先順位をテストで固定（実装は既存spread順を維持＝最小変更） |
| F121 | 堅牢性 | lib/features/text_download/data/download_service.dart:318-340 | Medium | M | エピソード取得はリトライなしの一発勝負。一時的503が即failedCountになる | ✅ 対応済み(add-download-transient-retry): リトライを全GETのchoke point `_fetchPageResponse` に一括追加（目次1ページ目・後続目次・エピソード本文を均一カバー）。対象は**HTTP 5xxとTimeoutExceptionのみ**で`maxRetries`(既定2)・`retryBaseDelay`(既定500ms)を注入可能、指数バックオフ。4xxは永続失敗として即throw、`ClientException`(キャンセル起因)は非リトライでキャンセル誤分類を回避、バックオフは`throwIfCancelled`でキャンセル協調。使い切り後は文脈別の既存契約(伝播/truncated/failedCount)を維持。逐次応答モック`sequencedClient`を新設しテスト13本追加、既存5xx/timeoutテストは`retryBaseDelay:0`注入でwall-clock回避(F123)。429はNon-Goal |
| F122 | テスト | test/features/text_download/（全アダプタテスト） | Medium | M | 実ページfixture（.html）がリポジトリに0件、全テストが極小合成HTML。さらに失敗系の検証もゼロ（`failedCount`を見るテストが1件もない）。スクレイパー最大の故障モード＝実マークアップドリフトと部分失敗をオフラインで検出できない | 🟡 一部対応: fix-download-silent-failuresで失敗系（empty_parse/timeout/truncated/cancellation）＋narouエピソードfixtureを追加。harden-scraper-parse-contractで各アダプタの**目次ページ**fixture（narou/kakuyomu/hameln/aozora のvalid＋narou/hameln drifted）と空目次→`EmptyIndexException`の失敗系を追加。⚠️ ただし追加したfixtureは**手書きの合成HTML**だったため、2026-08にハーメルンが目次を`episode-list`形式へ刷新した際も全テストが緑のまま本番だけが壊れた（fix-hameln-index-markup-drift）。同changeでハーメルンのfixtureを実ページ由来のスナップショットへ差し替え、旧マークアップは`hameln_index_legacy_table.html`として「もう解析されないこと」の回帰検体に転用した。残: 他サイト（narou/kakuyomu/aozora）のfixtureも実ページ由来へ、および各サイトの実エピソードページの網羅的スナップショット |
...
| F129 | テスト | lib/features/novel_metadata_db/data/novel_database.dart:238-246 | Medium | M | v3→v4ブックマーク移行（RENAME/再作成/コピー/DROP）のテストカバレッジゼロ。v1→v7のフルチェーン昇格テストもなく、`runMigrationForTesting`（:432-471）は_onUpgradeのステップ順序を迂回するため各versionブロックの相互作用が未検証 | ✅ 対応済み(harden-db-test-schema-fidelity): フルチェーン昇格はF128で既設、本changeで残るv3→v4を`novel_database_migration_v4_test`で固定。歴史的v3スキーマ（`line_number`なし）をシードし本番`NovelDatabase`昇格パス経由でpre-v4行が`line_number=NULL`付与のまま生存することを検証。**判明事項**: `_onUpgrade`は`oldVersion`のみで分岐し`newVersion`を見ないためv3→v4の単独分離は本番変更なしには不可（v8終端で観測）。`runMigrationForTesting`は迂回路を増やさぬよう不使用 |
| F130 | テスト | test/features/novel_delete/data/novel_delete_service_test.dart:38-80 ほか計5フィクスチャ | Medium | M | 本番DDLがテストに手書きコピーされ、`_onCreate`（novel_database.dart:98-120）とのスキーマドリフトがテストを通過して本番で壊れる構造 | ✅ 対応済み(harden-db-test-schema-fidelity): 本番`_onCreate`を`@visibleForTesting static NovelDatabase.createCurrentSchema`へ抽出し単一の正に。`test/helpers/novel_metadata_db_fixture.dart`（`openInMemoryNovelMetadataDb`/`seedNovelDatabaseFixture`）経由へ13テストファイルを移行し、現行スキーマの手書きDDLを一掃（残る手書きは歴史版シードの移行テストのみ＝本番非保持で正当）。`schema_fidelity_test`がフィクスチャと本番のスキーマ（テーブル＋インデックス）一致を固定しドリフトを構造的に検出。新spec`db-schema-test-fidelity` |
...
| F133 | アーキテクチャ | lib/features/llm_summary/data/llm_summary_pipeline.dart:22-48 + テスト14本 | Medium | M | `generate()`は本番で死にコード（serviceはextractFileFacts/summarizeFromFactsのみ使用）。14本のテストが死路をピン留めし、死んでいることを覆い隠す | ✅ 対応済み(purge-dead-code): `generate()`削除（共有ヘルパー`_extractFactsRecursive`/`_isolatedNotifier`/`_parseSummaryResponse`は現役のため残す）。`llm_summary_pipeline_test.dart`は全体が`generate()`専用のためファイルごと削除。共有ヘルパーの未カバー検証7本（再帰上限/無進捗停止/F132×2/非JSON fallback/コードフェンス/onProgress隔離）を`summarizeFromFacts`経由で`*_per_file_test.dart`へ移植。round=1進捗発火は新アーキで呼び出し側責務のため破棄 |
...
| F137 | アーキテクチャ | lib/features/llm_summary/data/llm_summary_service.dart:217-247 + presentation/analysis_runner.dart:191-247 + data/folder_file_lister.dart:6-12 | Medium | M | 「実効エピソード=数値プレフィクスなければ辞書順位」の不変条件が3箇所に独立実装され、警告コメント（"All three MUST agree…"）だけで束ねられている。ドリフトすると読書位置を超えたネタバレが漏れる | ✅ 対応済み(extract-episode-resolver): 「実効エピソード番号」導出とフォルダ`.txt`列挙を単一の共有モジュール`lib/shared/episode/episode_resolver.dart`（feature非依存・`lib/shared/`配下＝`novel_metadata_db`が`llm_summary`を逆参照しない位置）へ抽出。実際は**4箇所**の複製（analysis_runnerトリガ解決／llm_summary_serviceスコープフィルタ／novel_database v5 migrationの`_extractNumericPrefix`+`_lexicalRank`+独立列挙）を委譲へ統一し、`folder_file_lister.dart`は削除（"All three MUST agree"コメントも消滅）。振る舞い不変（2165テストgreen）。レビュー反映2件——`extractNumericPrefix`を`int.tryParse`化（20桁超ファイル名の`FormatException`クラッシュ防止）、`listSortedTextFileNames`に`onError`フック追加（v5 migrationの列挙失敗警告を復元）。新spec`episode-resolution`。残: `text_search_providers.dart`の5つ目の複製（表示ソート順＝別関心事）はNon-Goal |
...
| F139 | エラー処理 | lib/app/startup_migrations.dart:11 + lib/features/settings/data/settings_repository.dart:169 | Medium | S | APIキー移行失敗が`debugPrint`のみ＝releaseビルドではファイルログに残らない。キーが平文prefsに残留したユーザを診断できない | ✅ 対応済み(fix-update-path-diagnosability): 両箇所を`Logger.warning(e, stack)`(AppLoggerパイプライン)へ。移行の冪等性・非ブロッキング・失敗時ソース保持の契約は不変。`lib内`のdebugPrintはAppLogger実装自身のみになった。`llm-settings`specの移行要件に「WARNINGで記録しrelease痕跡を残す」を追加 |
| F140 | エラー処理 | lib/features/app_update/data/installer_downloader.dart:76（同類: distribution_detector.dart:31, installer_updater.dart:83, installer_verifier.dart:20, registry_reader.dart:23, update_dialog.dart:84） | Medium | S | update経路に文字通り空の`catch (_) {}`が6箇所。update_dialog.dart:69-74は実際の例外文言（UpdateResult.message）も捨てる。フィールドでの更新失敗が診断不能 | ✅ 対応済み(fix-update-path-diagnosability): 6箇所に`Logger('app_update.*')`を追加し制御フロー不変。**ログレベルを失敗の性質で切り分け**——真の異常(verifier読取不能/updater・downloaderクリーンアップ失敗/dialogブラウザ起動失敗)はWARNING、想定内フォールバック(distribution_detector→portable/registry_reader→null、ポータブル版で毎起動通る)はFINE(release閾値INFOでドロップ)。update_dialogは`_startUpdate`で捨てていた`UpdateResult.message`もWARNINGログ化。downloader外側catch(:71)はdialog層と重複するため非ログ。`app-update-check`specに診断可能性要件を追加 |
...
| F144 | 並行性 | lib/features/tts/data/tts_session.dart:89-115 + tts_isolate.dart:86-107 | Medium | M | `synthesize()`にタイムアウトなし、`Isolate.spawn`に`addErrorListener`なし。workerがDart例外で死ぬとcompleterが永遠に未解決→streamingが「waiting」のまま無限ハング | ✅ 対応済み(fix-tts-isolate-worker-death-hang): 新spec`tts-isolate-resilience`を立て、worker異常終了を`Isolate.spawn`の`onError`/`onExit`引数でアトミックに武装（spawn後の`addErrorListener`はSendPort送信前の死を取りこぼすため不採用）→`WorkerDiedResponse`をbroadcast。TtsSessionが`synthesize`/`ensureModelLoaded`で決定論的にエラー解決（null/false）。`_disposing`フラグで正常dispose誤検知回避、F111のhandle free規律も維持。レビューで判明した3件のハング再現穴（①死亡後の2回目操作=one-shot応答非再生→`hasWorkerDied`でfail-fast、②spawn前死亡→spawn completerエラー解決、③spawn登録レース→onError/onExitアトミック武装）も修正。modelLoadタイムアウト(既定120s)はバックストップ。合成への固定タイムアウトと自動復旧はNon-Goal |
...
| F146 | アーキテクチャ | lib/features/tts/data/tts_stored_player_controller.dart:13-123 | Medium | S | クラス全体が死にコード（lib内生成ゼロ、openspecアーカイブでも「既に死んでいる」と認知済み）。324行のテストも維持コスト | ✅ 対応済み(purge-dead-code): クラスと324行テストを削除。保存音声再生の実所有者は統合コントローラ`TtsStreamingController`（`bufferDrainDelay`→`SegmentPlayer`委譲）のため、`tts-stored-playback` spec の振る舞い要件は維持し、死クラスを名指しする「Audio buffer drain」要件2箇所のみ`TtsStreamingController`へ更新（capability削除はせず） |
...
| F156 | アーキテクチャ | lib/features/text_viewer/presentation/vertical_text_viewer.dart:238-434 | Medium | L | build()が約200行のスケジューラと化し、レイアウト中の状態変異（:252,:286,:274-277）+4種のポストフレームsetStateジャンプを振り付け。最高チャーンのロジック（31コミット/6ヶ月）が最もテスト困難な場所に集中し、`_scheduledTargetPage`等のガードは再入バグの後追いパッチ | ✅ 対応済み(refactor-vertical-viewer-effect-orchestration): 5効果（①ターゲット飛び②アニメ停止③最終ページ飛び④TTS自動ナビ⑤行レポート）を純関数`resolveViewerEffects(ViewerEffectInputs)→ViewerEffects`(コマンド値)へ持ち上げ、build()は単一`_applyViewerEffects`で適用するだけの薄い層に。ワンショットフラグ消費を同期化し再入による二重適用を構造的に排除、ジャンプ/hover/行コールバックは単一post-frameへ集約。ページネーション計算(F115/F116/F117)は対象外。新規ユニットテスト18本＋既存widgetテスト全green(2096)で振る舞い不変を担保。レビュー(code-review/codex)で判明した2件——hover-hideがno-opジャンプでも発火／①③を1フィールドに畳んだことで`fromEnd` intent消失の挙動変更——を、①③独立フィールド化＋hover-hideをジャンプ適用に内在させる形で修正。`vertical-text-display`スペックに「ページ効果の決定論的オーケストレーション」要件を追加 |
...
| F164 | アーキテクチャ | 死にコード一括: vertical_text_layout.dart:68-93（hitTestCharIndex+旧式テスト128-237）, swipe_detection.dart:27-66（detectSwipe）, ruby_text_parser.dart:91-103（buildPlainTextと_concatenatedBaseTextの重複）, text_content_renderer.dart:41-47（computeLineStartOffsets）, download_service.dart:81-84（fetchPage—再利用されると文字化け/403の罠）, llm_config.dart:14（isConfigured）, tts_edit_providers.dart:18-28 / tts_audio_repository.dart:133-140,195-202 / voice_recording_service.dart:40（テストのみ参照のメソッド群） | Low | S | 本番参照ゼロのコード＋それをピン留めする偽カバレッジテスト。特にhitTestCharIndexのテストは廃止済みグリッド幾何をassertし誤った安心感を与える | 🟡 一部対応(purge-dead-code): 高信頼の死シンボル4件——`fetchPage`(罠)・`hitTestCharIndex`・`detectSwipe`・`buildPlainText`——を専用テストごと削除（現役の兄弟`hitTestCharIndexFromRegions`/`detectSwipeFromDrag`/`_concatenatedBaseText`/`_fetchPageResponse`は保持）。**監査リストの陳腐化が判明**: `tts_audio_repository.dart:195-202`の`getGeneratedSegmentCount`は現在`tts_streaming_controller.dart:152`で**本番使用中**（削除せず）。`computeLineStartOffsets`は現役関数`measureCharOffsetY`テストのオラクル、`getSegmentCount`(:133-140)は現役`tts_edit_controller`テストのオラクルのため除外。`isConfigured`/`voice_recording_service.isRecording`/`tts_edit_providers updateSegment`は本番未参照だが無害な自然APIで一部は現役テストのオラクル＝今回はNon-Goal |
...
- [x] F139: 移行系の `debugPrint` 2件を `Logger` へ（change: fix-update-path-diagnosability）
- [x] F140: update経路の空catch 6箇所にロギング追加（change: fix-update-path-diagnosability。異常=WARNING/想定内フォールバック=FINEで切り分け）
- [x] F132: LLM応答の `is String` 検証（生JSON永続化の防止）（change: harden-llm-client）
- [x] F146: `TtsStoredPlayerController` とテスト324行を削除（change: purge-dead-code）
- [x] F164(一部): 死にコード4件削除（`fetchPage`(罠)/`hitTestCharIndex`/`detectSwipe`/`buildPlainText`、change: purge-dead-code。陳腐化/オラクル項目はNon-Goal）
...
- **`vertical_text_viewer.dart:252,286` のbuild内フィールド代入** — 古典的なsmellだが、キー/ホイールハンドラが現在のページネーションを同期的に必要とし、ページネーションはbuildでしか得られないconstraintsに依存する。問題は周囲のgod build（F156）であって代入自体ではない。 — ✅ 注: F156は解決済み(refactor-vertical-viewer-effect-orchestration)。`_pageCount`/`_currentPageSegments`/`_lastConstraints`等の同期代入は依然buildに残るが（ハンドラが要求する正当な用途）、5効果のスケジュール群は純関数`resolveViewerEffects`＋単一`_applyViewerEffects`へ外出しされ、god buildではなくなった。
...
6. **`qwen3_tts_abort` は解放済みctxに対して安全か**（third_party側は未監査）: 安全でなければF111はCritical相当に格上げ。~~🔬 調査要(2026-06-13): メンテナも安全性は不明と回答。third_party（qwen3-tts.cpp）の監査が必要。F124+F125 の後、先行spikeで判定してからF111の重大度を確定する。~~ ✅ 決着(2026-06-14, fix-tts-abort-use-after-free): spikeで `qwen3_tts_abort(ctx)` は `if(!ctx)return` の null ガードしか持たず `ctx->abort_flag.store(true)` で**書き込み**を行うと確認。`qwen3_tts_free` が `delete ctx` で flag ごと解放するため、再ロード窓の abort は**解放済みヒープへの UAF write**（read より重い）。**Critical 格上げ確定**。abort フラグを ctx と別ライフタイムの `qwen3_tts_abort_handle` へ分離して根治済み。
