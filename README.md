...
長押しはポインタの種別で区別していないため、マウスで長押ししても同じメニューが開きます。一方、縦書きの「選択範囲内のタップ」は指の場合に限られます——タップにはもともと選択解除という意味があり、マウス操作の挙動は変えていません。トラックパッドを接続した iPad では、従来どおり右クリックも使えます。

縦書きで選択範囲の外側をタップした場合は、これまでどおり選択が解除されます。ドラッグによる範囲選択とスワイプによるページ送りにも変更はありません。
